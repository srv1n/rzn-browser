...
extension_bundles_ready() {
  local target
  for target in chrome edge chromium; do
    if [[ ! -f "extension/dist/$target/manifest.json" ]]; then
      return 1
    fi
  done
  return 0
...
if [[ "$SKIP_RUST" != "1" ]]; then
  echo "Building Rust components (rzn-native-host + rzn-browser)..."
  build_rust() {
    if [[ "$PROFILE" == "release" ]]; then
      cargo build --release -p rzn-native-host -p rzn-browser
    else
      cargo build -p rzn-native-host -p rzn-browser
    fi
  }
...
if [[ "$SKIP_EXT" != "1" ]]; then
  if [[ "${RZN_SETUP_FORCE_EXT_BUILD:-0}" != "1" ]] && extension_bundles_ready; then
    echo "[INFO] extension/dist/{chrome,edge,chromium} already exists; skipping extension build"
    echo "       Set RZN_SETUP_FORCE_EXT_BUILD=1 to rebuild"
  else
    if ! command -v bun >/dev/null 2>&1; then
      if extension_bundles_ready; then
        echo "[WARN] bun is not installed; reusing existing extension/dist/{chrome,edge,chromium}"
      else
        echo "[ERROR] bun is required to build the extension, and extension/dist/{chrome,edge,chromium} is incomplete"
        exit 1
      fi
    else
    echo "Building Chrome, Edge, and Chromium extension bundles..."
    cd extension
    if [ -f "bun.lock" ]; then
      echo "Installing extension dependencies (bun install --frozen-lockfile)..."
...
if [ -n "$NATIVE_HOST_INSTALL_DIR" ]; then
  ...
  if [[ "$OSTYPE" == "darwin"* && "$PROFILE" == "debug" ]]; then
    echo "[INFO] Leaving debug CLI in the repo build dir on macOS."
  ...
  # Only strip downloaded launch metadata after the bootstrapper has verified the artifact checksum.
  if [[ "$OSTYPE" == "darwin"* ]]; then
    strip_launch_xattrs "$NATIVE_HOST_PATH"
    if [[ "$CLI_PATH" == "$NATIVE_HOST_INSTALL_DIR/"* ]]; then
      strip_launch_xattrs "$CLI_PATH"
    fi
  fi
fi
...
echo "Installing native host manifests for Chrome, Chromium, and Edge..."
"$CLI_PATH" native-host install \
  --browser chrome,chromium,edge \
  --native-host-path "$NATIVE_HOST_PATH"

echo "Installed native host registrations:"
"$CLI_PATH" native-host list --browser chrome,chromium,edge || true
...
echo "1. Open chrome://extensions/, edge://extensions/, or chromium://extensions/."
echo "2. Enable 'Developer mode'."
echo "3. Click 'Load unpacked' and select the matching extension directory below."
echo "4. Verify the extension ID is: $EXTENSION_ID"
echo "5. Restart the browser fully to ensure the Native Messaging Host is registered."
echo ""
if [[ "$INSTALL_GLOBAL_BIN" == "1" ]]; then
...
echo "  - Chrome:   $INSTALLED_EXTENSION_ROOT/chrome"
echo "  - Edge:     $INSTALLED_EXTENSION_ROOT/edge"
echo "  - Chromium: $INSTALLED_EXTENSION_ROOT/chromium"
echo "To run workflows (unified desktop wiring):"
echo "1. Open the target browser. The extension will connect to '$HOST_NAME', causing the browser to launch rzn-native-host."
echo "2. Run a workflow through the local supervisor:"
...
echo "7. Extension folders to load:"
echo "   Chrome:   $INSTALLED_EXTENSION_ROOT/chrome"
echo "   Edge:     $INSTALLED_EXTENSION_ROOT/edge"
echo "   Chromium: $INSTALLED_EXTENSION_ROOT/chromium"
