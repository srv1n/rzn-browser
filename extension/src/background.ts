...
async function loadPersistedCdpLeases(): Promise<PersistedCdpLease[]> {
  try {
    const value = await chrome.storage?.session?.get?.(CDP_LEASE_STORAGE_KEY);
    return normalizePersistedCdpLeases(value?.[CDP_LEASE_STORAGE_KEY]);
  } catch (error) {
    console.warn('[CDPLease] Failed to load persisted leases:', error);
    return [];
  }
}

async function savePersistedCdpLeases(leases: PersistedCdpLease[]): Promise<void> {
  try {
    await chrome.storage?.session?.set?.({ [CDP_LEASE_STORAGE_KEY]: leases });
  } catch (error) {
    console.warn('[CDPLease] Failed to persist leases:', error);
  }
}

// All persisted-lease writes go through one queue: load-modify-save from concurrent
// upserts and sweeps would otherwise overwrite each other's changes.
let cdpLeaseStorageMutationQueue: Promise<void> = Promise.resolve();

function mutatePersistedCdpLeases(
  mutate: (leases: PersistedCdpLease[]) => PersistedCdpLease[],
): Promise<void> {
  const run = async (): Promise<void> => {
    const leases = await loadPersistedCdpLeases();
    await savePersistedCdpLeases(mutate(leases));
  };
  cdpLeaseStorageMutationQueue = cdpLeaseStorageMutationQueue.then(run, run);
  return cdpLeaseStorageMutationQueue;
}

async function persistCdpLease(tabId: TabId, expiresAtMs: number, purpose = 'cdp'): Promise<void> {
  await mutatePersistedCdpLeases((leases) =>
    upsertPersistedCdpLease(leases, { tabId, expiresAtMs, purpose }),
  );
}

async function removePersistedCdpLease(tabId: TabId): Promise<void> {
  await mutatePersistedCdpLeases((leases) => removePersistedCdpLeaseByTab(leases, tabId));
}
...
async function sweepPersistedCdpLeases(): Promise<void> {
  const now = Date.now();
  const leases = await loadPersistedCdpLeases();
  const { active, expired } = splitCdpLeasesByExpiry(leases, now);

  for (const lease of active) {
    const current = leaseExpirations.get(lease.tabId) ?? 0;
    if (lease.expiresAtMs > current) {
      leaseExpirations.set(lease.tabId, lease.expiresAtMs);
      ensureLeaseTimer(lease.tabId);
    }
  }

  for (const lease of expired) {
    leaseExpirations.delete(lease.tabId);
    await frameRouter.detachFromTab(lease.tabId).catch(() => {
      console.warn(`[CDPLease] Failed to detach expired CDP lease for tab ${lease.tabId}`);
    });
  }

  if (expired.length) {
    await mutatePersistedCdpLeases((current) => splitCdpLeasesByExpiry(current, now).active);
  }
}

async function reconcilePersistedCdpLeases(): Promise<void> {
  const leases = await loadPersistedCdpLeases();
  const now = Date.now();

  for (const [tabId, expiresAtMs] of activeLeaseExpirationsAfterStartup(leases, now)) {
    leaseExpirations.set(tabId, expiresAtMs);
    ensureLeaseTimer(tabId);
  }

  const targets = await new Promise<any[]>((resolve) => {
    try {
      chrome.debugger?.getTargets?.((items) => {
        if (chrome.runtime.lastError) {
          console.warn('[CDPLease] debugger.getTargets failed:', chrome.runtime.lastError.message);
          resolve([]);
          return;
        }
        resolve(items || []);
      ...
      console.warn('[CDPLease] debugger.getTargets threw:', error);
      resolve([]);
    }
  });
  ...
  await sweepPersistedCdpLeases();
}

function initializeCdpLeaseDurability(): void {
  ensureCdpLeaseSweepAlarm();
  void reconcilePersistedCdpLeases();
}
...
async function maybeExpireCdpLease(tabId: TabId, intervalId: number): Promise<void> {
  ...
  await withCdpLock(async () => {
    ...
    await removePersistedCdpLease(tabId);
    await frameRouter.detachFromTab(tabId).catch(() => {
...
function setCdpLeaseExpiration(tabId: TabId, expiresAtMs: number) {
  ...
  const next = Math.max(prev, expiresAtMs);
  leaseExpirations.set(tabId, next);
  void persistCdpLease(tabId, next);
  ensureCdpLeaseSweepAlarm();
  ensureLeaseTimer(tabId);
}
...
interface ExtensionResponse {
  ...
  title?: string;
  text?: string;
  id_url_map?: Record<string, string>;
  count?: number;
}

let nativePort: chrome.runtime.Port | null = null;
let nativePortEpoch = 0;
let lastNativePortDisconnectReason: string | null = null;
const EXTENSION_WORKER_BOOT_ID =
...
function initializeBrowserInstanceId(): void {
  void ensureBrowserInstanceId().catch((error: any) => {
    logWarn('Failed to initialize browser instance id', {
...
async function callNativeHostControl(
  ...
  const requestNativePortEpoch = nativePortEpoch;
  ...
  return await new Promise<BrokerMessage>((resolve, reject) => {
    ...
    nativeControlCallbacks.set(reqId, {
      ...
      nativePortEpoch: requestNativePortEpoch,
      resolve,
      reject,
      timeoutId,
    });
    ...
    try {
      if (nativePort !== requestPort || nativePortEpoch !== requestNativePortEpoch) {
        throw new Error('native port epoch changed before control request was sent');
      }
...
let lastNativeRoundtripPingMs: number | null = null;
const suppressedNativeDisconnectPorts = new WeakSet<chrome.runtime.Port>();
const HEARTBEAT_INTERVAL_MS = 10000;
const HEARTBEAT_TIMEOUT_MS = 5000;
const HEARTBEAT_MISSES_BEFORE_RECONNECT = 2;
const BROKER_WATCHDOG_DEFAULT_MS = 30_000;
...
let lastNativeHostPid: number | null = null;
...
const brokerRequestLeases = new Map<string, BrokerRequestLease>();
const brokerRequestLeasesById = new Map<string, BrokerRequestLease>();
const brokerRequestLeaseKeysByRequestId = new Map<string, Set<string>>();
...
function scheduleReconnectAlarm(delayMs: number) {
  ...
  try {
    if (!chrome.alarms?.create) {
      console.warn('[NativeReconnect] chrome.alarms unavailable; reconnect may not survive service worker suspension');
      return;
    }
...
function disconnectNativePort(reason: string): void {
  ...
  const disconnectedEpoch = nativePortEpoch;
  nativePortEpoch += 1;
  lastNativePortDisconnectReason = reason;
  suppressedNativeDisconnectPorts.add(port);
  cancelBrokerLeasesForNativePortEpoch(disconnectedEpoch, reason);
  nativePort = null;
...
function isNativeHostStdoutHeartbeat(message: BrokerMessage): boolean {
  return message?.cmd === NATIVE_HOST_STDOUT_HEARTBEAT_CMD;
}
...
function handleNativeHostStdoutHeartbeat(message: BrokerMessage): void {
  ...
  lastNativeHostBootId =
    typeof message.payload?.native_host_boot_id === 'string'
      ? message.payload.native_host_boot_id
      : lastNativeHostBootId;
  lastNativeHostPid =
    typeof message.payload?.native_host_pid === 'number'
      ? message.payload.native_host_pid
      : lastNativeHostPid;
  console.debug('[NativeKeepalive] Native-host stdout heartbeat', {
    seq: message.payload?.seq,
    interval_ms: message.payload?.interval_ms,
    native_host_pid: message.payload?.native_host_pid,
    native_host_boot_id: message.payload?.native_host_boot_id,
  });
}

function markBrokerRequestTimedOut(requestId: string, lease?: BrokerRequestLease | null): void {
  const staleSet = lease ? brokerWatchdogTimedOutLeaseIds : brokerWatchdogTimedOutRequestIds;
  const key = lease?.leaseId || requestId;
  staleSet.add(key);
  setTimeout(() => {
    staleSet.delete(key);
  }, WATCHDOG_STALE_RESPONSE_TTL_MS);
}
...
function createMissingWorkflowTabError(sessionId: string, action: string): Error {
  const error = new Error(buildMissingWorkflowTabError(sessionId, action));
  (error as any).code = 'NO_WORKFLOW_TAB';
  return error;
}

function isMissingWorkflowTabError(error: unknown): error is Error & { code: 'NO_WORKFLOW_TAB' } {
  return typeof error === 'object' && error !== null && (error as any).code === 'NO_WORKFLOW_TAB';
}
...
function getWorkflowSessionState(sessionId: string): WorkflowSessionState {
  ...
  if (!state) {
    state = { brokerEpoch: 0, queue: Promise.resolve() };
...
function brokerRequestId(message: BrokerMessage): string | undefined {
...
function brokerRequestLeaseKey(requestId: string, leaseId: string): string {
  return `${requestId}:${leaseId}`;
}

function indexBrokerRequestLease(lease: BrokerRequestLease): void {
  const key = brokerRequestLeaseKey(lease.requestId, lease.leaseId);
  brokerRequestLeases.set(key, lease);
  brokerRequestLeasesById.set(lease.leaseId, lease);
  let keys = brokerRequestLeaseKeysByRequestId.get(lease.requestId);
  if (!keys) {
    keys = new Set();
    brokerRequestLeaseKeysByRequestId.set(lease.requestId, keys);
  }
  keys.add(key);
}

function brokerLeaseIsRegistered(lease: BrokerRequestLease): boolean {
  return brokerRequestLeasesById.get(lease.leaseId) === lease;
}

function activeBrokerLeasesForRequestId(requestId: string): BrokerRequestLease[] {
  const keys = brokerRequestLeaseKeysByRequestId.get(requestId);
  if (!keys) {
    return [];
  }
  const leases: BrokerRequestLease[] = [];
  for (const key of Array.from(keys)) {
    const lease = brokerRequestLeases.get(key);
    if (lease) {
      leases.push(lease);
    } else {
      keys.delete(key);
  ...
  if (keys.size === 0) {
    brokerRequestLeaseKeysByRequestId.delete(requestId);
  }
  return leases;
}

function resolveBrokerLeaseForResponse(
  correlationId: string | undefined,
  responseLeaseId: string | undefined
): BrokerRequestLease | undefined {
  if (responseLeaseId) {
    const lease = brokerRequestLeasesById.get(responseLeaseId);
    if (lease) {
      return lease;
    }
  }
  ...
  const leases = activeBrokerLeasesForRequestId(correlationId);
  if (leases.length === 1) {
    return leases[0];
  }
  if (leases.length > 1) {
    console.warn('[NativeReconnect] Ambiguous response correlation; require lease_id to match active request', {
      req_id: correlationId,
      active_lease_ids: leases.map((lease) => lease.leaseId),
    });
  }
  return undefined;
}

function registerBrokerRequestLease(
  message: BrokerMessage,
  sessionId: string,
  sourceNativePortEpoch: number = nativePortEpoch,
  watchdogMs: number = brokerMessageWatchdogMs(message)
): BrokerRequestLease | null {
  const requestId = brokerRequestId(message);
  if (!requestId) {
    return null;
  }
  ...
  const leaseId = createBrokerLeaseId();
  const lease: BrokerRequestLease = {
    leaseId,
    requestId,
    command: brokerCommandName(message),
    nativePortEpoch: sourceNativePortEpoch,
    workflowSessionId,
    workflowSessionEpoch: getWorkflowSessionState(workflowSessionId).brokerEpoch,
    startedAtMs: Date.now(),
    deadlineAtMs: Date.now() + watchdogMs,
    cancelled: false,
    abortController,
    watchdogTimedOut: false,
  };
  indexBrokerRequestLease(lease);
  return lease;
}

function clearBrokerRequestLease(lease: BrokerRequestLease | null): void {
  if (!lease) {
    return;
  }
  const key = brokerRequestLeaseKey(lease.requestId, lease.leaseId);
  if (brokerRequestLeases.get(key) === lease) {
    brokerRequestLeases.delete(key);
  }
  const keys = brokerRequestLeaseKeysByRequestId.get(lease.requestId);
  if (keys) {
    keys.delete(key);
    if (keys.size === 0) {
      brokerRequestLeaseKeysByRequestId.delete(lease.requestId);
    }
  }
  if (brokerRequestLeasesById.get(lease.leaseId) === lease) {
    brokerRequestLeasesById.delete(lease.leaseId);
  }
}

function cancelBrokerRequestLease(lease: BrokerRequestLease | null, reason: string): void {
  if (!lease || lease.cancelled) {
    return;
  }
  lease.cancelled = true;
  lease.cancelReason = reason;
  try {
    lease.abortController.abort(reason);
  } catch {
    lease.abortController.abort();
  }
  const tabId = getWorkflowTabId(lease.workflowSessionId);
  if (tabId !== undefined) {
    void chrome.tabs.sendMessage(tabId, {
      cmd: 'RZN_CANCEL_REQUEST',
      req_id: lease.requestId,
...
function cancelBrokerLeasesForNativePortEpoch(portEpoch: number, reason: string): void {
  for (const lease of Array.from(brokerRequestLeasesById.values())) {
    if (lease.nativePortEpoch === portEpoch) {
      cancelBrokerRequestLease(lease, reason);
      invalidateWorkflowSessionEpoch(lease.workflowSessionId, reason);
    }
  }
}

function brokerLeaseIsCurrent(lease: BrokerRequestLease | null): boolean {
  if (!lease) {
    return true;
  }
  if (lease.cancelled) {
  ...
  if (lease.nativePortEpoch !== nativePortEpoch) {
    return false;
  }
  return lease.workflowSessionEpoch === getWorkflowSessionState(lease.workflowSessionId).brokerEpoch;
}

function assertBrokerLeaseCurrent(lease: BrokerRequestLease | null, where: string): void {
  if (brokerLeaseIsCurrent(lease)) {
    return;
  }
  const reason = lease?.cancelReason || 'broker request lease is stale';
  throw new Error(`${reason} (${where})`);
}

function brokerLeaseAbortError(lease: BrokerRequestLease, where: string): Error {
  const signalReason = (lease.abortController.signal as any)?.reason;
  const reason = lease.cancelReason || (typeof signalReason === 'string' ? signalReason : null) || 'broker request lease aborted';
  const error = new Error(`${reason} (${where})`);
  (error as any).name = 'AbortError';
  return error;
}

function createBrokerLeaseAbortRace<T>(
  lease: BrokerRequestLease | null,
  where: string
): { promise: Promise<T>; cleanup: () => void } | null {
  if (!lease) {
    return null;
  }
  const signal = lease.abortController.signal;
  if (signal.aborted) {
    return {
      promise: Promise.reject(brokerLeaseAbortError(lease, where)),
      cleanup: () => {},
    };
  }
  ...
  const promise = new Promise<T>((_, reject) => {
    listener = () => reject(brokerLeaseAbortError(lease, where));
...
function cleanupLateBrokerSideEffectResult(
  lease: BrokerRequestLease | null,
  where: string,
  result: unknown
): void {
  ...
  if (lowerWhere.includes('chrome.tabs.create') && typeof value?.id === 'number') {
    void chrome.tabs.remove(value.id).catch((error) => {
      console.debug('[NativeReconnect] Late created tab cleanup failed', {
        where,
        req_id: lease?.requestId,
        lease_id: lease?.leaseId,
        tab_id: value.id,
  ...
  if (lowerWhere.includes('chrome.windows.create') && typeof value?.id === 'number') {
    void chrome.windows.remove(value.id).catch((error) => {
      console.debug('[NativeReconnect] Late created window cleanup failed', {
        where,
        req_id: lease?.requestId,
        lease_id: lease?.leaseId,
        window_id: value.id,
...
async function guardedBrokerSideEffect<T>(
  lease: BrokerRequestLease | null,
  where: string,
  operation: () => Promise<T>
): Promise<T> {
  assertBrokerLeaseCurrent(lease, `before ${where}`);
  const operationPromise = operation();
  const abortRace = createBrokerLeaseAbortRace<T>(lease, where);
  if (!abortRace) {
    const result = await operationPromise;
    assertBrokerLeaseCurrent(lease, `after ${where}`);
    return result;
  }
  ...
  try {
    const result = await Promise.race([trackedOperation, abortRace.promise]);
    assertBrokerLeaseCurrent(lease, `after ${where}`);
    return result;
  } catch (error) {
    if (!operationSettled) {
      trackedOperation
        .then((lateResult) => {
          cleanupLateBrokerSideEffectResult(lease, where, lateResult);
        })
        .catch((lateError) => {
          console.debug('[NativeReconnect] Ignored late side-effect completion after broker lease abort', {
            where,
            req_id: lease?.requestId,
            lease_id: lease?.leaseId,
            error: lateError?.message || String(lateError),
          });
        });
    }
    throw error;
...
function onBrokerLeaseAbort(
  ...
  const listener = () => {
    ...
    void Promise.resolve(cleanup()).catch((error) => {
      console.debug('[NativeReconnect] Broker lease abort cleanup failed', {
        req_id: lease.requestId,
        lease_id: lease.leaseId,
        error: error?.message || String(error),
      });
...
function commitWorkflowTabIdForLease(
  lease: BrokerRequestLease | null,
  setSessionWorkflowTabId: (tabId: number | undefined) => void,
...
function attachBrokerLeaseToMessage<T extends BrokerMessage>(
  message: T,
  lease: BrokerRequestLease | null
): T {
  if (!lease) {
  ...
  return {
    ...
    rzn: {
      ...
      native_port_epoch: lease.nativePortEpoch,
      workflow_session_epoch: lease.workflowSessionEpoch,
    },
    payload: {
      ...(message.payload || {}),
      lease_id: message.payload?.lease_id || lease.leaseId,
    },
  };
}

function invalidateWorkflowSessionEpoch(sessionId: string, reason: string): number {
  const normalized = normalizeSessionId(sessionId);
  const state = getWorkflowSessionState(normalized);
  state.brokerEpoch += 1;
  console.warn('[NativeReconnect] Invalidated workflow session broker epoch', {
    session_id: normalized,
    broker_epoch: state.brokerEpoch,
    reason,
  });
  return state.brokerEpoch;
}

function messageEnvelopeUsesCurrentTab(message: BrokerMessage): boolean {
  return (
    message.payload?.use_current_tab === true ||
    message.payload?.use_active_tab === true ||
    message.data?.use_current_tab === true ||
    message.data?.use_active_tab === true
  );
}

function messageStepUsesCurrentTab(message: BrokerMessage): boolean {
  return (
    message.payload?.step?.use_current_tab === true ||
    message.payload?.step?.use_active_tab === true ||
    message.data?.step?.use_current_tab === true ||
    message.data?.step?.use_active_tab === true
  );
}

async function resolveSessionCommandTab(
  workflowSessionId: string,
  workflowTabId: number | undefined,
  setSessionWorkflowTabId: (tabId: number | undefined) => void,
  action: string,
  mayUseCurrentTab: boolean,
  brokerLease: BrokerRequestLease | null = null
): Promise<{ tabId: number; usedCurrentTab: boolean }> {
  assertBrokerLeaseCurrent(brokerLease, `resolve ${action} start`);
  if (workflowTabId !== undefined) {
    try {
      const tab = await guardedBrokerSideEffect(
        brokerLease,
        `chrome.tabs.get ${action}`,
        () => chrome.tabs.get(workflowTabId)
      );
      if (tab) {
        return { tabId: workflowTabId, usedCurrentTab: false };
      }
      commitWorkflowTabIdForLease(
        brokerLease,
        setSessionWorkflowTabId,
        undefined,
        `clear missing workflow tab for ${action}`
      );
    } catch {
      commitWorkflowTabIdForLease(
        brokerLease,
        setSessionWorkflowTabId,
        undefined,
        `clear inaccessible workflow tab for ${action}`
      );
    }
  }

  if (!mayUseCurrentTab) {
    throw createMissingWorkflowTabError(workflowSessionId, action);
  }

  const tabId = await guardedBrokerSideEffect(
    brokerLease,
    `get active tab for ${action}`,
    () => getActiveTabIdOrThrow(action)
  );
  commitWorkflowTabIdForLease(
    brokerLease,
    setSessionWorkflowTabId,
    tabId,
    `bind active workflow tab for ${action}`
  );
  return { tabId, usedCurrentTab: true };
}

async function resolveSessionCommandTabId(
  workflowSessionId: string,
  workflowTabId: number | undefined,
  setSessionWorkflowTabId: (tabId: number | undefined) => void,
  action: string,
  ...
  const { tabId } = await resolveSessionCommandTab(
    workflowSessionId,
    workflowTabId,
    setSessionWorkflowTabId,
    action,
...
async function getAxTreeForSession(
  message: BrokerMessage,
  workflowSessionId: string,
  workflowTabId: number | undefined,
  setSessionWorkflowTabId: (tabId: number | undefined) => void,
  ...
  try {
    ...
    const mayUseCurrentTab =
      sessionMayUseActiveTab(workflowSessionId) || messageEnvelopeUsesCurrentTab(message);
    const tabId = await resolveSessionCommandTabId(
      workflowSessionId,
      workflowTabId,
      setSessionWorkflowTabId,
      'get_ax_tree',
    ...
    if (!(await guardedBrokerSideEffect(
      brokerLease,
      'isCDPEnabledForTab get_ax_tree',
      () => isCDPEnabledForTab(tabId)
    ))) {
      return {
        success: false,
        error_code: 'CDP_DISABLED',
        error: 'CDP is disabled for this tab/domain',
      };
    }
    ...
    return {
      ...
      text,
      id_url_map: idUrlMap,
      count: nodesOut.length,
    };
  } catch (error: any) {
    console.error('get_ax_tree failed:', error);
    if (isMissingWorkflowTabError(error)) {
      return {
        success: false,
    ...
    return {
      success: false,
      error_code: 'GET_AX_TREE_ERROR',
      error: error?.message || String(error),
    };
  }
}

async function setFlagsForMessage(message: BrokerMessage): Promise<{
  success: boolean;
  error?: string;
}> {
  try {
...
async function withMessageTimeout<T>(promise: Promise<T>, timeoutMs: number, label: string): Promise<T> {
...
async function sendMessageTopFrameWithRetry<T = any>(
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      return await withMessageTimeout(
        sendMessageTopFrame<T>(tabId, message),
        timeoutMs,
        `content script message ${message?.cmd || 'unknown'}`
      );
    } catch (e: any) {
      lastErr = e;
...
// Steps like wait_for_element poll up to step.timeout_ms in the content script.
// The message-passing layer must outlive that, otherwise we kill the call before
// the content script can answer. Floor at 8s for cheap steps with no declared
// timeout; for declared timeouts, add a small grace window for the round-trip.
const STEP_MESSAGE_TIMEOUT_FLOOR_MS = 8000;
const STEP_MESSAGE_TIMEOUT_GRACE_MS = 5000;
function messageTimeoutForStep(step: any): number {
  const declared = Number(step?.timeout_ms ?? step?.timeoutMs);
  if (!Number.isFinite(declared) || declared <= 0) {
    return STEP_MESSAGE_TIMEOUT_FLOOR_MS;
  }
  return Math.max(STEP_MESSAGE_TIMEOUT_FLOOR_MS, Math.floor(declared) + STEP_MESSAGE_TIMEOUT_GRACE_MS);
}
...
async function resolveMessageTargetTab(
  ...
  return await getActiveBrowserTabId('message target');
}
...
function isContentScriptDisconnectError(error: unknown): boolean {
  ...
  return (
    ...
    msg.includes('Cannot inject on non-http(s) URL') ||
...
function isExecutionContextDestroyedError(error: unknown): boolean {
  return isExecutionContextDestroyedCdpError(error);
}

function executionContextDestroyedError(error: unknown): Error {
  const original = cdpErrorText(error);
  const suggestion =
    'The page likely navigated, re-rendered, or detached the debugger target while the script was awaiting. Keep ordinary scripts JS-first; for trusted actions, split state-changing clicks into short CDP evals with a wait_for_timeout after them, or use direct download actions instead of UI save clicks.';
...
const DEFAULT_SCRIPT_EVAL_TIMEOUT_MS = 8_000;
const SCRIPT_EVAL_POLL_MS = 250;

type ScriptEvalSettled =
  | { status: 'fulfilled'; value: chrome.scripting.InjectionResult<any>[] }
  | { status: 'rejected'; reason: any };

function normalizeScriptEvalTimeoutMs(value: any): number {
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    return DEFAULT_SCRIPT_EVAL_TIMEOUT_MS;
  }
  return Math.min(120_000, Math.max(1_000, Math.round(parsed)));
}

async function readTabNavigationState(tabId: number): Promise<TabNavigationState> {
  try {
    const tab = await chrome.tabs.get(tabId);
    return {
      url: tab.url || '',
    ...
    return { url: '', pendingUrl: '', status: '' };
  }
}

function sleepMs(ms: number): Promise<null> {
  return new Promise(resolve => setTimeout(() => resolve(null), ms));
}

function navigationObservedEvalResult(
  requestedWorld: string,
  wantsReturn: boolean,
  tabId: number,
  before: TabNavigationState,
  after: TabNavigationState,
  elapsedMs: number,
): {
  success: true;
  ...
  return {
    success: true,
    execution_backend: 'chrome_scripting_main_world_navigation_observed',
    requested_world: requestedWorld,
    result: wantsReturn
      ? {
          navigation_observed: true,
          before_url: before.url || before.pendingUrl || '',
          current_url: after.url || '',
          pending_url: after.pendingUrl || '',
          tab_status: after.status || '',
...
async function injectedRznScriptingEval(
  ...
  try {
  const buildInjectedScriptEvalBody = (rawScript: string): string => {
    const source = String(rawScript || '');
    const trimmed = source.trim();
    if (!trimmed) return '';
    if (/^return\b/.test(trimmed)) return source;
    const expressionSource = trimmed.replace(/^;+/, '').trimStart();
    const expressionLike =
      expressionSource.startsWith('(') ||
      expressionSource.startsWith('[') ||
      expressionSource.startsWith('{') ||
      /^async\s*\(/.test(expressionSource) ||
      /^function\b/.test(expressionSource);
    const statementLike =
      !expressionLike &&
  ...
  const body = buildInjectedScriptEvalBody(script);
  ...
  // Fast path: AsyncFunction wrapper. Subject to page CSP `unsafe-eval`.
  let runError: any = null;
  try {
    const AsyncFunction = Object.getPrototypeOf(async function () {}).constructor as FunctionConstructor;
    const run = new AsyncFunction('args', 'params', wrapperBody);
  ...
  const errMsg = (runError && (runError.message || String(runError))) || '';
  const isCspBlock = /unsafe-eval|EvalError|Content Security Policy/i.test(errMsg);
  if (!isCspBlock) throw runError;

  // Page CSP forbids `unsafe-eval` (e.g. chatgpt.com). Fall back to a
  // <script nonce=...> injection — script elements whose nonce matches the
  // page's CSP execute even when eval/Function are forbidden.
  const findNonce = (): string | null => {
  ...
  const value = await new Promise<any>((resolve, reject) => {
    ...
    const cleanup = () => {
      ...
      try { delete w[channelId + '_args']; } catch {}
      try { delete w[channelId + '_params']; } catch {}
      try { delete w[channelId + '_resolve']; } catch {}
      try { delete w[channelId + '_reject']; } catch {}
      try { delete w[channelId + '_started']; } catch {}
    };

    w[channelId + '_args'] = args;
    w[channelId + '_params'] = params;
    w[channelId + '_resolve'] = (v: any) => {
      cleanup();
      resolve(v);
    };
    w[channelId + '_reject'] = (e: any) => {
      cleanup();
...
async function runScriptingEval(
  ...
  const senderTabId = sender?.tab?.id;
  const explicitTabId = senderTabId === undefined && typeof message?.tabId === 'number'
    ? message.tabId
    : undefined;
  const tabId = senderTabId ?? explicitTabId ?? await getActiveBrowserTabId('script eval');
  if (tabId === undefined) {
  ...
  const evalTimeoutMs = normalizeScriptEvalTimeoutMs(message?.timeout_ms ?? message?.timeoutMs);
  ...
  const beforeNavigation = await readTabNavigationState(tabId);

  try {
    const scriptEvalPromise: Promise<ScriptEvalSettled> = chrome.scripting.executeScript({
        target: { tabId },
        world,
        func: injectedRznScriptingEval,
        args: [script, args, params, wantsReturn, RZN_EVAL_ERROR_KEY],
    } as chrome.scripting.ScriptInjection<any[], any>)
      .then(value => ({ status: 'fulfilled' as const, value }))
      .catch(reason => ({ status: 'rejected' as const, reason }));

    const startedAt = Date.now();
    let settled: ScriptEvalSettled | null = null;

    while (Date.now() - startedAt < evalTimeoutMs) {
      const remainingMs = Math.max(0, evalTimeoutMs - (Date.now() - startedAt));
      settled = await Promise.race([
        scriptEvalPromise,
        sleepMs(Math.min(SCRIPT_EVAL_POLL_MS, remainingMs)),
      ]);

      if (settled) break;

      const currentNavigation = await readTabNavigationState(tabId);
      if (tabNavigationChanged(beforeNavigation, currentNavigation)) {
        return navigationObservedEvalResult(
          requestedWorld,
          wantsReturn,
          tabId,
          beforeNavigation,
    ...
    if (!settled) {
      ...
      throw new Error(`SCRIPT_EVAL_TIMEOUT: chrome.scripting eval did not finish within ${evalTimeoutMs}ms`);
    }

    if (settled.status === 'rejected') {
      if (isExecutionContextDestroyedError(settled.reason)) {
        const currentNavigation = await readTabNavigationState(tabId);
        if (tabNavigationChanged(beforeNavigation, currentNavigation)) {
          return navigationObservedEvalResult(
            requestedWorld,
        ...
        throw executionContextDestroyedError(settled.reason);
      }
      throw settled.reason;
    }

    const [result] = settled.value;
    const evalError = evalErrorFromScriptingResult(result);
    if (evalError) {
      throw evalError;
    }
...
async function runCdpEval(
  ...
  const explicitTabId = senderTabId === undefined && typeof message?.tabId === 'number'
    ? message.tabId
    : undefined;
  const tabId = senderTabId ?? explicitTabId ?? await getActiveBrowserTabId('CDP eval');
  if (tabId === undefined) {
  ...
  const params =
    message?.params && typeof message.params === 'object' && !Array.isArray(message.params)
      ...
      : {};
  const wantsReturn = message?.return_value !== false;
  const body = buildScriptEvalBody(script);
...
function brokerMessageWatchdogMs(message: BrokerMessage): number {
  const raw = Number(
    message.timeout_ms ??
    message.timeoutMs ??
    message.payload?.timeout_ms ??
    message.payload?.timeoutMs ??
    message.payload?.step?.timeout_ms ??
    message.payload?.step?.timeoutMs
  );
  if (Number.isFinite(raw) && raw > 0) {
    return Math.max(1_000, raw - BROKER_WATCHDOG_GRACE_MS);
  }
...
async function handleBrokerMessageWithWatchdog(
  ...
  ) => Promise<void> = handleBrokerMessage
): Promise<void> {
  const watchdogMs = brokerMessageWatchdogMs(message);
  const workflowSessionId = normalizeSessionId(sessionId || resolveSessionId(message));
  const lease = registerBrokerRequestLease(message, workflowSessionId, sourceNativePortEpoch, watchdogMs);
  if (sourceNativePortEpoch !== nativePortEpoch) {
    cancelBrokerRequestLease(lease, 'native port epoch became stale before queued work started');
    clearBrokerRequestLease(lease);
    return;
  }
  let settled = false;
  let timeoutId: number | undefined;
  ...
  const watchdog = new Promise<void>((resolve) => {
    timeoutId = setTimeout(() => {
      ...
      const requestId = message.req_id || message.task_id;
      console.error('[NativeReconnect] Broker handler watchdog timeout', {
        cmd: message.cmd || message.action,
        req_id: requestId,
        session_id: workflowSessionId,
        native_port_epoch: lease?.nativePortEpoch ?? nativePortEpoch,
        watchdog_ms: watchdogMs,
      });
      if (lease) {
        lease.watchdogTimedOut = true;
      }
      cancelBrokerRequestLease(lease, 'broker handler watchdog timeout');
      lastBrokerWatchdogTimeoutMs = Date.now();
      lastBrokerWatchdogRequestId = requestId || null;
      lastBrokerWatchdogSessionId = workflowSessionId;
      invalidateWorkflowSessionEpoch(workflowSessionId, 'broker handler watchdog timeout');
      if (requestId) {
        markBrokerRequestTimedOut(requestId, lease);
        sendResponseToBroker({
          req_id: requestId,
          lease_id: lease?.leaseId,
          success: false,
          error_code: 'BROKER_WATCHDOG_TIMEOUT',
          error_msg: `Extension broker handler timed out after ${watchdogMs}ms`,
  ...
  const handler = (async () => {
    try {
      assertBrokerLeaseCurrent(lease, 'before broker handler');
      await handlerFn(message, workflowSessionId, lease);
    } finally {
      settled = true;
      if (timeoutId !== undefined) {
        clearTimeout(timeoutId);
  ...
  const winner = await Promise.race([
    handler.then(() => 'handler' as const),
    watchdog.then(() => 'watchdog' as const),
  ]);
  if (winner === 'watchdog') {
    void handler.catch((error) => {
      console.warn('[NativeReconnect] Cancelled broker handler unwound after watchdog', {
        req_id: lease?.requestId,
        lease_id: lease?.leaseId,
        error: error?.message || String(error),
      });
...
function connectToNative(): void {
  ...
  try {
    ...
    const port = connectNative(host);
    nativePort = port;
    nativePortEpoch += 1;
    const portEpoch = nativePortEpoch;

    // Set the native port for the logger
    setNativePort(port);

    if (guardListener(port?.onMessage, 'nativePort.onMessage')) {
      port.onMessage.addListener((message: BrokerMessage) => {
        if (nativePort !== port || portEpoch !== nativePortEpoch) {
          console.warn('[NativeReconnect] Dropping message from stale native port epoch', {
            message_cmd: message?.cmd || message?.action,
            message_req_id: message?.req_id || message?.task_id,
            port_epoch: portEpoch,
            current_epoch: nativePortEpoch,
          });
          return;
        }
        if (isNativeHostStdoutHeartbeat(message)) {
          handleNativeHostStdoutHeartbeat(message);
          return;
        }
        console.log('Received from broker:', {
          cmd: message?.cmd || message?.action,
    ...
    if (guardListener(port?.onDisconnect, 'nativePort.onDisconnect')) {
      port.onDisconnect.addListener(() => {
        const err = chrome.runtime.lastError?.message;
        const suppressed = suppressedNativeDisconnectPorts.has(port);
        if (nativePort !== port) {
          console.debug('[NativeReconnect] Ignoring stale native port disconnect', {
            error: err,
            host: brokerHostInUse,
            suppressed,
          });
          return;
        }
        ...
        const disconnectedEpoch = portEpoch;
        nativePortEpoch += 1;
        lastNativePortDisconnectReason = err || 'Native host disconnected';
        cancelBrokerLeasesForNativePortEpoch(disconnectedEpoch, err || 'Native host disconnected');
...
function handleDeltaMessage(message: BrokerMessage): void {
  if (message.action === 'dom_delta' || message.cmd === 'apply_dom_delta') {
    console.log('Received DOM delta message:', {
      cmd: message.cmd || message.action,
      req_id: message.req_id || message.task_id,
...
const sendResponseToBrokerRaw = sendResponseToBroker;
type BrokerCommandRegistration = {
  command: string;
  source: 'native_broker';
  dispatch: string;
};

const BROKER_COMMAND_REGISTRY = new Map<string, BrokerCommandRegistration>([
  'apply_dom_delta',
  'cdp_action',
  'detect_auto_list',
  'disable_debug',
  ...
  'set_execution_tier',
  'set_flags',
].map((command) => [command, { command, source: 'native_broker', dispatch: command }]));

function getBrokerCommandRegistration(
  command: string | undefined,
  message: BrokerMessage
): BrokerCommandRegistration | undefined {
  if (!command) return undefined;
  const registered = BROKER_COMMAND_REGISTRY.get(command);
  if (registered) return registered;
  if (message.rzn && typeof message.cmd === 'string' && message.cmd === command) {
    return { command, source: 'native_broker', dispatch: command };
  }
  return undefined;
}
...
async function handleBrokerMessage(
  ...
  if (requestedTabId !== undefined) {
    commitWorkflowTabIdForLease(
      ...
      requestedTabId,
      'bind requested workflow tab'
    );
  }
  const setSessionWorkflowTabId = (tabId: number | undefined) => {
    assertBrokerLeaseCurrent(brokerLease, 'before workflow tab id commit');
    workflowTabId = tabId;
    setWorkflowTabId(workflowSessionId, tabId);
    assertBrokerLeaseCurrent(brokerLease, 'after workflow tab id commit');
  };
  const sendResponseToBroker = (
    response: ExtensionResponse,
    options: {
      allowAfterWatchdog?: boolean;
      allowStalePortEpoch?: boolean;
      lease?: BrokerRequestLease | null;
    } = {}
  ) => {
  ...
  const commandRegistration = getBrokerCommandRegistration(command, message);
  if (!commandRegistration) {
    sendResponseToBroker({
      ...
      error_msg: `Unknown broker command: ${command || '<missing>'}`,
    });
    return;
  }
  const registeredCommand = commandRegistration.dispatch;

  console.log('Handling broker message:', registeredCommand, 'req_id:', requestId, 'orchestrator format:', isOrchestratorFormat);
  console.log('Message has task:', !!message.task);
  console.log('Broker message summary:', {
    cmd: registeredCommand,
    req_id: requestId,
    has_task: !!message.task,
    task_step_count: Array.isArray(message.task?.steps) ? message.task.steps.length : undefined,
  });
  ...
  if (registeredCommand === 'ping') {
    const browserInstanceIdentity = await browserInstancePingMetadata().catch((error: any) => {
      logWarn('Failed to resolve browser instance id for ping response', {
        error: error?.message || String(error),
      });
      return {
        ...extensionRuntimePingMetadata(),
        browser_instance_id: getCachedBrowserInstanceId(),
      };
    });
    sendResponseToBroker({
      req_id: isOrchestratorFormat ? undefined : requestId,
      ...
      result: {
        ...
        supervisor_bridge_id: message.rzn_bridge?.supervisor_bridge_id || null,
        supervisor_bridge_epoch: message.rzn_bridge?.supervisor_bridge_epoch ?? null,
        native_host_boot_id: message.rzn_bridge?.native_host_boot_id || lastNativeHostBootId,
        native_host_pid: message.rzn_bridge?.native_host_pid ?? lastNativeHostPid,
        native_host_connected: !!nativePort,
        native_connect_in_flight: nativeConnectInFlight,
        native_port_epoch: nativePortEpoch,
        last_native_port_disconnect_reason: lastNativePortDisconnectReason,
        last_native_host_stdout_heartbeat_ms: lastNativeHostStdoutHeartbeatMs,
        last_native_host_stdout_heartbeat_seq: lastNativeHostStdoutHeartbeatSeq,
        last_native_host_stdout_heartbeat_age_ms:
          lastNativeHostStdoutHeartbeatMs === null ? null : Date.now() - lastNativeHostStdoutHeartbeatMs,
        last_native_roundtrip_ping_ms: lastNativeRoundtripPingMs,
        last_native_roundtrip_ping_age_ms:
          lastNativeRoundtripPingMs === null ? null : Date.now() - lastNativeRoundtripPingMs,
        missed_native_roundtrip_pings: missedNativeHeartbeats,
        broker_active_request_count: brokerRequestLeasesById.size,
        ...
        capabilities: {
          ...
          content_keepalive_interval_ms: CONTENT_KEEPALIVE_EXPECTED_INTERVAL_MS,
          native_keepalive_alarm: !!chrome.alarms?.create,
  ...
  if (registeredCommand === 'session_close') {
    try {
      ...
      console.error('session_close failed:', error);
      sendResponseToBroker({
        req_id: isOrchestratorFormat ? undefined : requestId,
        task_id: isOrchestratorFormat ? requestId : undefined,
        success: false,
  ...
  if (registeredCommand === 'enable_debug') {
    try {
      ...
      sendResponseToBroker({
        ...
        capabilities: await guardedBrokerSideEffect(
          brokerLease,
          'buildCapabilities enable_debug',
          () => buildCapabilities(tabId)
        ),
      } as any);
      return;
    } catch (error) {
  ...
  if (registeredCommand === 'disable_debug') {
    try {
      const mayUseCurrentTab =
        sessionMayUseActiveTab(workflowSessionId) || messageEnvelopeUsesCurrentTab(message);
      const tabId = await resolveSessionCommandTabId(
      ...
      sendResponseToBroker({
        ...
        capabilities: await guardedBrokerSideEffect(
          brokerLease,
          'buildCapabilities disable_debug',
          () => buildCapabilities(tabId)
        ),
      } as any);
      return;
  ...
  if (registeredCommand === 'execute_workflow' && message.task) {
    // Handle workflow execution from orchestrator
    logInfo('Routing to executeWorkflow', { hasTask: true });
    await executeWorkflow(message, requestId, isOrchestratorFormat, workflowSessionId, brokerLease);
  } else if (registeredCommand === 'execute_workflow' && !message.task) {
    ...
    (registeredCommand === 'execute_step' || (registeredCommand === 'execute_workflow' && message.payload?.step)) &&
    message.payload?.step?.type === 'get_page_source'
  ) {
    try {
      const mayUseCurrentTab =
        sessionMayUseActiveTab(workflowSessionId) ||
        messageEnvelopeUsesCurrentTab(message) ||
        messageStepUsesCurrentTab(message);
      const tabId = await resolveSessionCommandTabId(
        workflowSessionId,
        workflowTabId,
        setSessionWorkflowTabId,
      ...
      const [result] = await guardedBrokerSideEffect(
        brokerLease,
        'chrome.scripting.executeScript get_page_source',
  ...
  } else if (registeredCommand === 'eval_with_cdp') {
    try {
      const payload = message.payload || {};
      const mayUseCurrentTab =
        sessionMayUseActiveTab(workflowSessionId) || messageEnvelopeUsesCurrentTab(message);
      const { tabId, usedCurrentTab } = await resolveSessionCommandTab(
        workflowSessionId,
        workflowTabId,
        setSessionWorkflowTabId,
        'eval_with_cdp',
      ...
      if (usedCurrentTab) {
        await guardedBrokerSideEffect(
          brokerLease,
          'waitForTabComplete eval_with_cdp active tab',
          () => waitForTabComplete(tabId, 15000).catch(() => {})
        );
      }

      const evalResponse = await guardedBrokerSideEffect(
        brokerLease,
        'runCdpEval eval_with_cdp',
        () => runCdpEval(undefined, {
      ...
      const tab = await guardedBrokerSideEffect(
        brokerLease,
        'chrome.tabs.get eval_with_cdp response',
        () => chrome.tabs.get(tabId)
      );
      sendResponseToBroker({
        req_id: isOrchestratorFormat ? undefined : requestId,
  ...
  } else if (registeredCommand === 'execute_step' || (registeredCommand === 'execute_workflow' && message.payload?.step)) {
    // Forward to content script - create tab if needed
    try {
      assertBrokerLeaseCurrent(brokerLease, 'execute_step start');
      let tabId: number | undefined;
      const step = message.payload?.step;
      const stepType = typeof step?.type === 'string' ? step.type : '';
      const requiresExistingTab =
        stepType !== 'open_new_tab' &&
        stepType !== 'navigate_to_url' &&
      ...
      if (workflowTabId !== undefined && requiresExistingTab) {
        try {
          const tab = await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.get existing workflow tab',
            () => chrome.tabs.get(workflowTabId)
          );
          if (tab) {
            tabId = workflowTabId;
          ...
          commitWorkflowTabIdForLease(
            ...
            setSessionWorkflowTabId,
            undefined,
            'clear inaccessible workflow tab'
          );
        }
      }
      ...
      if (workflowTabId === undefined && requiresExistingTab) {
        if (mayUseCurrentTab) {
          ...
          tabId = await guardedBrokerSideEffect(
            brokerLease,
            'get active tab for execute_step',
            () => getActiveTabIdOrThrow('use_current_tab execute_step')
          );
          commitWorkflowTabIdForLease(
            brokerLease,
            setSessionWorkflowTabId,
            tabId,
            'bind active workflow tab'
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete active workflow tab',
            () => waitForTabComplete(tabId!, 15000).catch(() => {})
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'ensureContentReady active workflow tab',
          ...
          try {
            ...
            // window exists. Open a window first, then use its tab.
            const msg = err?.message || String(err);
            if (!/No current window|No window with id/i.test(msg)) throw err;
            console.log('No current window — creating one for the workflow');
            const newWindow = await guardedBrokerSideEffect(
              brokerLease,
              'chrome.windows.create workflow window',
              () => chrome.windows.create({ url: 'https://www.example.com', focused: true })
            );
            const firstTab = newWindow.tabs?.[0];
            if (!firstTab?.id) throw new Error(`Created window but no tab available: ${msg}`);
            newTab = firstTab;
          }
          tabId = newTab.id!;
          commitWorkflowTabIdForLease(
            ...
            setSessionWorkflowTabId,
            tabId,
            'bind dedicated workflow tab'
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete dedicated workflow tab',
            () => waitForTabComplete(tabId!, 15000).catch(() => {})
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'ensureContentReady dedicated workflow tab',
            () => ensureContentReady(tabId!, 'contentScript.js', 8000).catch(() => {})
          );
        }
      } else {
      ...
      if (step?.type === 'open_new_tab') {
        try {
          ...
          commitWorkflowTabIdForLease(
            ...
            setSessionWorkflowTabId,
            tabId,
            'bind open_new_tab workflow tab'
          );

          if (shouldPrepareContent) {
            await guardedBrokerSideEffect(
              brokerLease,
              'waitForTabUrl open_new_tab',
              () => waitForTabUrl(tabId!, candidateUrl => isHttpNavigableUrl(candidateUrl), 15000).catch(() => {})
            );
            const openedTab = await guardedBrokerSideEffect(
              brokerLease,
              'chrome.tabs.get open_new_tab',
            ...
            if (isHttpNavigableUrl(openedUrl)) {
              await guardedBrokerSideEffect(
                brokerLease,
                'waitForTabComplete open_new_tab',
                () => waitForTabComplete(tabId!, 15000).catch(() => {})
              );
              await guardedBrokerSideEffect(
                brokerLease,
                'ensureContentReady open_new_tab',
                () => ensureContentReady(tabId!, 'contentScript.js', 8000).catch(() => {})
              );
            }
          }
          ...
          const openedTab = await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.get open_new_tab response',
            () => chrome.tabs.get(tabId!)
          );
          sendResponseToBroker({
            req_id: isOrchestratorFormat ? undefined : requestId,
            task_id: isOrchestratorFormat ? requestId : undefined,
            success: true,
            result: { tabId, tab_id: tabId, opened: true, url: openedTab.url || openedTab.pendingUrl || url },
      ...
      if (step?.type === 'navigate_to_url') {
        try {
          ...
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForNavigation navigate_to_url',
            () => waitForNavigation(tabId!, step.wait, step.timeout_ms || step.timeoutMs || 15000)
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'ensureContentReady navigate_to_url',
            () => ensureContentReady(tabId!, 'contentScript.js', 8000).catch(() => {})
          );
      ...
      if (step?.type === 'get_current_url') {
        try {
          const tab = await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.get get_current_url',
            () => chrome.tabs.get(tabId!)
          );
          sendResponseToBroker({
            req_id: isOrchestratorFormat ? undefined : requestId,
      ...
      if (
        ...
        try {
          ...
          const evalResponse = await guardedBrokerSideEffect(
            brokerLease,
            `execute ${wantsCdpEval ? 'CDP' : 'scripting'} eval`,
            () => evalRunner(undefined, {
              script: step.script,
              args: step.args,
              params: step.params,
      ...
      if (step?.type === 'switch_to_tab') {
        try {
          ...
          await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.get switch_to_tab',
            () => chrome.tabs.get(targetTabId!)
          );

          await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.update switch_to_tab',
          ...
          commitWorkflowTabIdForLease(
            ...
            setSessionWorkflowTabId,
            targetTabId,
            'bind switch_to_tab workflow tab'
          );

          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete switch_to_tab',
            () => waitForTabComplete(targetTabId!, 15000).catch(() => {})
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'ensureContentReady switch_to_tab',
            () => ensureContentReady(targetTabId!, 'contentScript.js', 8000).catch(() => {})
          );
      ...
      if (step?.type === 'wait_for_navigation') {
        try {
          ...
          const url = await guardedBrokerSideEffect(
            brokerLease,
            'waitForNavigationOrUrlMatch',
            () => waitForNavigationOrUrlMatch(tabId, urlPattern, timeoutMs)
          );
      ...
      if (step?.type === 'wait_for_network_idle') {
        try {
          ...
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete wait_for_network_idle',
            () => waitForTabComplete(tabId, maxWait).catch(() => {})
          );
          const spent = Date.now() - start;
          const remaining = Math.max(0, maxWait - spent);
          await guardedBrokerSideEffect(
            brokerLease,
            'idle delay wait_for_network_idle',
            () => new Promise(r => setTimeout(r, Math.min(idle, remaining)))
          );
      ...
      if (step?.type === 'upload_file') {
        try {
          ...
          await guardedBrokerSideEffect(
            brokerLease,
            'forceDetachCDP upload_file',
            () => forceDetachCDP(tabId).catch(() => {
      ...
      if (step?.type === 'take_screenshot') {
        try {
          let dataUrl = await guardedBrokerSideEffect(
            brokerLease,
          ...
          if (wantAnnotate) {
            try {
              ...
              await guardedBrokerSideEffect(
                brokerLease,
                'waitForTabComplete screenshot annotate',
                () => waitForTabComplete(tabId, 15000).catch(() => {})
              );
              await guardedBrokerSideEffect(
                brokerLease,
                'ensureContentReady screenshot annotate',
                () => ensureContentReady(tabId, 'contentScript.js', 8000).catch(() => {})
              );

              const domResp: any = await guardedBrokerSideEffect(
                brokerLease,
                'sendMessageTopFrameWithRetry screenshot annotate',
                () => sendMessageTopFrameWithRetry(
                  tabId,
                  {
                    cmd: 'get_dom_snapshot',
                    req_id: `${requestId}-dom`,
                    lease_id: brokerLease?.leaseId,
      ...
      try {
        await guardedBrokerSideEffect(
          brokerLease,
          'ensureContentReady content-script dispatch',
          () => ensureContentReady(tabId, 'contentScript.js', 8000)
        );
        const response = await guardedBrokerSideEffect(
          brokerLease,
          'sendMessageTopFrameWithRetry content-script dispatch',
          () => sendMessageTopFrameWithRetry(tabId, attachBrokerLeaseToMessage(message, brokerLease), {
            attempts: 3,
            timeoutMs: messageTimeoutForStep(step),
          })
        );
        sendResponseToBroker(response, { lease: brokerLease });
      } catch (error: any) {
    ...
    try {
      ...
      if (workflowTabId !== undefined) {
        try {
          const tab = await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.get DOM processing workflow tab',
            () => chrome.tabs.get(workflowTabId!)
          );
          if (tab) {
            tabId = workflowTabId;
            ...
            commitWorkflowTabIdForLease(brokerLease, setSessionWorkflowTabId, undefined, 'clear missing DOM workflow tab');
          }
        } catch (e) {
          commitWorkflowTabIdForLease(brokerLease, setSessionWorkflowTabId, undefined, 'clear inaccessible DOM workflow tab');
        }
      }
      ...
      if (workflowTabId === undefined) {
        if (mayUseCurrentTab) {
          tabId = await guardedBrokerSideEffect(
            brokerLease,
            'get active tab for DOM processing',
            () => getActiveTabIdOrThrow('dom processing')
          );
          commitWorkflowTabIdForLease(brokerLease, setSessionWorkflowTabId, tabId, 'bind DOM workflow tab');
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete DOM processing',
            () => waitForTabComplete(tabId, 15000).catch(() => {})
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'ensureContentReady DOM processing',
            () => ensureContentReady(tabId, 'contentScript.js', 8000).catch(() => {})
          );
        } else {
          const newTab = await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.create DOM processing',
            () => chrome.tabs.create({ url: 'https://www.example.com', active: true })
          );
          tabId = newTab.id!;
          commitWorkflowTabIdForLease(brokerLease, setSessionWorkflowTabId, tabId, 'bind DOM workflow tab');
      ...
      const targetTab = await guardedBrokerSideEffect(
        brokerLease,
        'chrome.tabs.get DOM request target',
        () => chrome.tabs.get(tabId)
      );
      const targetUrl = tabContentTargetUrl(targetTab);
      if (!isContentScriptTargetUrl(targetUrl)) {
        sendResponseToBroker({
          req_id: isOrchestratorFormat ? undefined : requestId,
          task_id: isOrchestratorFormat ? requestId : undefined,
          success: false,
          error_code: 'INVALID_PAGE',
          error_msg: invalidContentScriptTargetMessage(targetUrl, 'inspect DOM'),
      ...
      await guardedBrokerSideEffect(
        brokerLease,
        'ensureContentReady DOM request',
        () => ensureContentReady(tabId, 'contentScript.js', 8000)
      );
      ...
      const response = await guardedBrokerSideEffect(
        brokerLease,
        'sendMessageTopFrameWithRetry DOM request',
        () => sendMessageTopFrameWithRetry(tabId, attachBrokerLeaseToMessage(message, brokerLease), {
          attempts: 3,
          timeoutMs: 8000,
        })
      ...
      (response as any).current_tab_id = (response as any).current_tab_id ?? tabId;
      try {
        (response as any).capabilities = await guardedBrokerSideEffect(
          brokerLease,
          'buildCapabilities DOM request',
          () => buildCapabilities(tabId)
        );
      } catch (e) {
      ...
      if (isContentScriptDisconnectError(error)) {
        const errorMessage = (error as Error)?.message || String(error);
        console.warn(`DOM request could not reach content script: ${errorMessage}`);
        sendResponseToBroker({
          req_id: isOrchestratorFormat ? undefined : requestId,
          task_id: isOrchestratorFormat ? requestId : undefined,
          success: false,
  ...
  } else if (registeredCommand === 'observe') {
    ...
    try {
      ...
      if (workflowTabId !== undefined) {
        try {
          const tab = await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.get observe workflow tab',
            () => chrome.tabs.get(workflowTabId!)
          );
          if (tab) {
            tabId = workflowTabId;
          } else {
            commitWorkflowTabIdForLease(brokerLease, setSessionWorkflowTabId, undefined, 'clear missing observe workflow tab');
          }
        } catch {
          commitWorkflowTabIdForLease(brokerLease, setSessionWorkflowTabId, undefined, 'clear inaccessible observe workflow tab');
        }
      }
      if (workflowTabId === undefined) {
        if (mayUseCurrentTab) {
          tabId = await guardedBrokerSideEffect(
            brokerLease,
            'get active tab for observe',
            () => getActiveTabIdOrThrow('observe')
          );
          commitWorkflowTabIdForLease(brokerLease, setSessionWorkflowTabId, tabId, 'bind observe workflow tab');
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete observe',
            () => waitForTabComplete(tabId, 15000).catch(() => {})
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'ensureContentReady observe',
            () => ensureContentReady(tabId, 'contentScript.js', 8000).catch(() => {})
          );
        } else {
          const newTab = await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.create observe',
            () => chrome.tabs.create({ url: 'https://www.example.com', active: true })
          );
          tabId = newTab.id!;
          commitWorkflowTabIdForLease(brokerLease, setSessionWorkflowTabId, tabId, 'bind observe workflow tab');
      ...
      await guardedBrokerSideEffect(
        brokerLease,
        'ensureContentReady observe request',
        () => ensureContentReady(tabId, 'contentScript.js', 8000).catch(() => {})
      );

      const tab = await guardedBrokerSideEffect(
        brokerLease,
        'chrome.tabs.get observe current tab',
        () => chrome.tabs.get(tabId)
      );
      ...
      let domHashResp: any;
      try {
        domHashResp = await guardedBrokerSideEffect(
          brokerLease,
          'sendMessageTopFrame observe get_dom_hash',
          () => sendMessageTopFrame(tabId, attachBrokerLeaseToMessage(
            { cmd: 'get_dom_hash', req_id: `${requestId}-hash` },
            brokerLease
          ))
        );
      } catch (e) {
      ...
      let response: any;
      try {
        response = await guardedBrokerSideEffect(
          brokerLease,
          'sendMessageTopFrame observe execute_step',
          () => sendMessageTopFrame(tabId, attachBrokerLeaseToMessage(
            { cmd: 'execute_step', req_id: requestId, payload: { step } },
            brokerLease
          ))
        );
      } catch (e: any) {
  ...
  } else if (registeredCommand === 'get_active_tab') {
  ...
  } else if (registeredCommand === 'send_to_tab') {
    ...
    try {
      ...
      const response = await guardedBrokerSideEffect(
        brokerLease,
        'sendMessageTopFrame send_to_tab',
        () => sendMessageTopFrame(tabId, attachBrokerLeaseToMessage(tabMessage, brokerLease))
      );
      sendResponseToBroker({
        req_id: requestId,
        success: true,
  ...
  } else if (registeredCommand === 'cdp_action') {
    ...
    try {
      ...
      if (workflowTabId !== undefined) {
        ...
        if (mayUseCurrentTab) {
          tabId = await guardedBrokerSideEffect(
            brokerLease,
            'get active tab for cdp_action',
            () => getActiveTabIdOrThrow('cdp_action')
          );
          commitWorkflowTabIdForLease(brokerLease, setSessionWorkflowTabId, tabId, 'bind cdp_action workflow tab');
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete cdp_action',
            () => waitForTabComplete(tabId, 15000).catch(() => {})
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'ensureContentReady cdp_action',
            () => ensureContentReady(tabId, 'contentScript.js', 8000).catch(() => {})
          );
        } else {
          const newTab = await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.create cdp_action',
            () => chrome.tabs.create({ url: 'https://www.example.com', active: true })
          );
          tabId = newTab.id!;
          commitWorkflowTabIdForLease(brokerLease, setSessionWorkflowTabId, tabId, 'bind cdp_action workflow tab');
  ...
  } else if (registeredCommand === 'get_ax_tree') {
    const response = await getAxTreeForSession(
      message,
      workflowSessionId,
      workflowTabId,
      setSessionWorkflowTabId,
    ...
    sendResponseToBroker({
      ...
      result: response.success
        ? {
            text: response.text,
            id_url_map: response.id_url_map,
            count: response.count,
          }
        : undefined,
      text: response.success ? response.text : undefined,
      id_url_map: response.success ? response.id_url_map : undefined,
      count: response.success ? response.count : undefined,
    } as any);
  } else if (registeredCommand === 'set_flags') {
    const response = await setFlagsForMessage(message);
    sendResponseToBroker({
      req_id: requestId,
      success: response.success,
      error_msg: response.error,
    } as any);
  } else if (registeredCommand === 'get_cdp_context') {
    ...
    try {
      ...
      if (!(await guardedBrokerSideEffect(
        brokerLease,
        'isCDPEnabledForTab get_cdp_context',
        () => isCDPEnabledForTab(tabId)
      ))) {
        sendResponseToBroker({
          req_id: requestId,
  ...
  } else if (registeredCommand === 'set_execution_tier') {
  ...
  } else if (registeredCommand === 'get_interactive_elements') {
    try {
      const mayUseCurrentTab =
        sessionMayUseActiveTab(workflowSessionId) || messageEnvelopeUsesCurrentTab(message);
      const tabId = await resolveSessionCommandTabId(
      ...
      if (!(await guardedBrokerSideEffect(
        brokerLease,
        'isCDPEnabledForTab get_interactive_elements',
        () => isCDPEnabledForTab(tabId)
      ))) {
        sendResponseToBroker({
          req_id: requestId,
    ...
    try {
      ...
      const response = await guardedBrokerSideEffect(
        brokerLease,
        'sendMessageTopFrame static action',
        () => sendMessageTopFrame(tabId, attachBrokerLeaseToMessage(message, brokerLease))
      );
      sendResponseToBroker({
        req_id: requestId,
...
function brokerResponseLogSummary(response: ExtensionResponse, lease?: BrokerRequestLease | null): Record<string, unknown> {
  const result = (response as any).result;
  return {
    req_id: response.req_id,
    task_id: response.task_id,
    lease_id: response.lease_id || lease?.leaseId,
...
function sendResponseToBroker(
  ...
  const correlationId = getResponseCorrelationId(response);
  const lease = options.lease || resolveBrokerLeaseForResponse(correlationId, response.lease_id);
  if (
    options.lease &&
    correlationId &&
    !brokerLeaseIsRegistered(options.lease) &&
    activeBrokerLeasesForRequestId(correlationId).length > 0 &&
    !options.allowAfterWatchdog
  ) {
    console.warn('[NativeReconnect] Dropping response whose request id was reused by a newer lease', {
      req_id: response.req_id,
      task_id: response.task_id,
      response_lease_id: options.lease.leaseId,
      active_lease_ids: activeBrokerLeasesForRequestId(correlationId).map((activeLease) => activeLease.leaseId),
    });
    return;
  }
  if (lease?.cancelled && !options.allowAfterWatchdog) {
    console.warn('[NativeReconnect] Dropping response from cancelled broker lease', {
      req_id: response.req_id,
      task_id: response.task_id,
      success: response.success,
      error_code: response.error_code,
      lease_id: lease.leaseId,
      cancel_reason: lease.cancelReason,
      command: lease.command,
    });
    return;
  }
  if (
    (
      (lease && brokerWatchdogTimedOutLeaseIds.has(lease.leaseId)) ||
      (!lease && correlationId && brokerWatchdogTimedOutRequestIds.has(correlationId))
    ) &&
    !options.allowAfterWatchdog
  ) {
    console.warn('[NativeReconnect] Dropping stale response after broker watchdog timeout', {
      req_id: response.req_id,
      task_id: response.task_id,
      success: response.success,
      error_code: response.error_code,
  ...
  if (
    lease &&
    lease.nativePortEpoch !== nativePortEpoch &&
    !options.allowStalePortEpoch
  ) {
    console.warn('[NativeReconnect] Dropping stale response from previous native port epoch', {
      req_id: response.req_id,
      task_id: response.task_id,
      success: response.success,
      error_code: response.error_code,
      request_epoch: lease.nativePortEpoch,
      current_epoch: nativePortEpoch,
      command: lease.command,
    });
    return;
  ...
  if (
    lease &&
    lease.workflowSessionEpoch !== getWorkflowSessionState(lease.workflowSessionId).brokerEpoch &&
    !options.allowAfterWatchdog
  ) {
    console.warn('[NativeReconnect] Dropping stale response from invalidated workflow session epoch', {
      req_id: response.req_id,
      task_id: response.task_id,
      success: response.success,
      error_code: response.error_code,
      session_id: lease.workflowSessionId,
      request_session_epoch: lease.workflowSessionEpoch,
      current_session_epoch: getWorkflowSessionState(lease.workflowSessionId).brokerEpoch,
      command: lease.command,
    });
    return;
  }
  ...
  if (nativePort) {
    ...
    if (response.task_id || response.action) {
      ...
      console.log('Sending orchestrator response to broker:', brokerResponseLogSummary(response, lease));
      postToNative(orchestratorResponse);
    } else {
      // Extension format - keep as is
      console.log('Sending extension response to broker:', brokerResponseLogSummary(response, lease));
...
if (chrome.debugger?.onDetach?.addListener) {
  chrome.debugger.onDetach.addListener((source) => {
    ...
    leaseExpirations.delete(tabId);
    void removePersistedCdpLease(tabId);
  });
}
...
if (chrome.alarms?.onAlarm?.addListener) {
  chrome.alarms.onAlarm.addListener((alarm) => {
    if (alarm?.name === CDP_LEASE_SWEEP_ALARM_NAME) {
      void sweepPersistedCdpLeases();
      return;
    }
...
if (chrome.runtime?.onConnect?.addListener) {
  chrome.runtime.onConnect.addListener((port) => {
    if (port.name !== CONTENT_KEEPALIVE_PORT_NAME) {
      return;
    }
    ...
    const ensureNativeFromContentPort = (reason: string) => {
      ensureNativeKeepaliveAlarm();
      if (!nativePort) {
        console.log('[NativeKeepalive] Content port wake; native port missing, attempting reconnect', {
          reason,
          tabId: senderTabId,
      ...
      const now = Date.now();
      if (now - lastAckMs < CONTENT_KEEPALIVE_ACK_MIN_INTERVAL_MS) {
        return;
      }
      lastAckMs = now;
      try {
        port.postMessage({
          type: 'RZN_CONTENT_KEEPALIVE_ACK',
          nativeHostConnected: !!nativePort,
          nativeConnectInFlight,
          host: brokerHostInUse || null,
          ts: now,
        });
      } catch {
        // The page side may have navigated away; onDisconnect will clean up.
      }
    };

    ensureNativeFromContentPort('port_open');

    port.onMessage.addListener((message) => {
      if (message?.type === 'RZN_CONTENT_KEEPALIVE') {
        ensureNativeFromContentPort(message.reason || 'port_heartbeat');
      }
    });

    port.onDisconnect.addListener(() => {
      const err = chrome.runtime.lastError?.message;
      console.debug('[NativeKeepalive] Content keepalive port disconnected', {
        tabId: senderTabId,
        frameId: senderFrameId,
        error: err,
      });
  ...
  console.warn('[RZN] chrome.runtime.onConnect not available in this context');
}
...
if (RZN_PAGE_TEST_BRIDGE_ENABLED) {
  // Expose test functions only in explicit dev/test extension builds.
...
function normalizeTestBrokerMessage(message: any): { normalizedMessage: any; correlationId: string } {
  ...
  return { normalizedMessage, correlationId };
}

function waitForTestBrokerResponse(correlationId: string, timeoutMs = 20000): Promise<ExtensionResponse> {
...
if (RZN_PAGE_TEST_BRIDGE_ENABLED) {
  ...
  const responsePromise = waitForTestBrokerResponse(correlationId);
  ...
  const testHandler =
    normalizedMessage.cmd === '__rzn_test_sleep_then_mutate'
      ? async (
          ...
          _workflowSessionId: string,
          brokerLease: BrokerRequestLease | null
        ): Promise<void> => {
          const requestId = brokerMessage.req_id || brokerMessage.task_id || correlationId;
          const sleepMsRaw = Number(brokerMessage.payload?.sleep_ms ?? brokerMessage.payload?.sleepMs ?? 0);
          const sleepMs = Number.isFinite(sleepMsRaw) ? Math.max(0, sleepMsRaw) : 0;
          await new Promise(resolve => setTimeout(resolve, sleepMs));
          assertBrokerLeaseCurrent(brokerLease, 'before __rzn_test_sleep_then_mutate mutation');
          const currentCount = Number((globalThis as any).__rznTestLateMutationCount || 0);
          (globalThis as any).__rznTestLateMutationCount = currentCount + 1;
...
async function executeWorkflow(
  ...
  const workflowSessionId = normalizeSessionId(sessionId);
  const sendWorkflowResponse = (response: ExtensionResponse): void => {
    const leasedResponse =
      brokerLease?.leaseId && !response.lease_id
        ? { ...response, lease_id: brokerLease.leaseId }
        : response;
    sendResponseToBroker(leasedResponse, { lease: brokerLease });
  };

  console.log('Executing workflow from orchestrator', {
    req_id: requestId,
    has_task: !!message.task,
    step_count: Array.isArray(message.task?.steps) ? message.task.steps.length : undefined,
  });
  ...
  if (requiresExistingTab && workflowTabId === undefined) {
    ...
    try {
      if (preferCurrentTab) {
        ...
        workflowTabId = await guardedBrokerSideEffect(
          brokerLease,
          'get active workflow tab',
          () => getActiveTabIdOrThrow('use_current_tab workflow')
        );
        commitWorkflowTabIdForLease(
          brokerLease,
          setSessionWorkflowTabId,
        ...
        try {
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete active workflow tab',
            () => waitForTabComplete(workflowTabId!, 15000).catch(() => {})
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'ensureContentReady active workflow tab',
            () => ensureContentReady(workflowTabId!, 'contentScript.js', 8000)
          );
        } catch (e) {
        ...
        try {
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete dedicated workflow tab',
            () => waitForTabComplete(workflowTabId!, 15000)
          );
  ...
  try {
    ...
    for (let i = 0; i < steps.length; i++) {
      assertBrokerLeaseCurrent(brokerLease, `executeWorkflow step ${i + 1} start`);
      const step = steps[i];
      console.log(`Executing step ${i + 1}/${steps.length}:`, step.type);

      // Create a step execution message
      const stepMessage = attachBrokerLeaseToMessage(
        {
          cmd: 'execute_step',
      ...
      if (step.type === 'open_new_tab') {
        try {
          ...
          commitWorkflowTabIdForLease(
            ...
            setSessionWorkflowTabId,
            tabId,
            'bind workflow open_new_tab'
          );

          if (shouldPrepareContent) {
            await guardedBrokerSideEffect(
              brokerLease,
              'waitForTabUrl workflow open_new_tab',
              () => waitForTabUrl(tabId, candidateUrl => isHttpNavigableUrl(candidateUrl), 15000).catch(() => {})
            );

            const openedTab = await guardedBrokerSideEffect(
              brokerLease,
              'chrome.tabs.get workflow open_new_tab',
              () => chrome.tabs.get(tabId).catch(() => undefined)
            );
            const openedUrl = getTabNavigationUrl(openedTab);
            if (isHttpNavigableUrl(openedUrl)) {
              await guardedBrokerSideEffect(
                brokerLease,
                'waitForTabComplete workflow open_new_tab',
                () => waitForTabComplete(tabId, 15000).catch(() => {})
              );
              await guardedBrokerSideEffect(
                brokerLease,
                'ensureContentReady workflow open_new_tab',
                () => ensureContentReady(tabId, 'contentScript.js', 8000).catch(() => {})
              );
            } else {
              console.warn(
          ...
          results.push({ type: 'open_new_tab', url, tabId, tab_id: tabId, success: true });
          continue;
        } catch (error) {
        ...
        try {
          ...
          await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.get workflow switch_to_tab',
            () => chrome.tabs.get(targetTabId!)
          );

          await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.update workflow switch_to_tab',
            () => chrome.tabs.update(targetTabId!, { active: true })
          );
          commitWorkflowTabIdForLease(
            brokerLease,
            setSessionWorkflowTabId,
            targetTabId,
            'bind workflow switch_to_tab'
          );

          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete workflow switch_to_tab',
            () => waitForTabComplete(targetTabId!, 15000).catch(() => {})
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'ensureContentReady workflow switch_to_tab',
            () => ensureContentReady(targetTabId!, 'contentScript.js', 8000).catch(() => {})
          );
        ...
        try {
          ...
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForNavigation workflow navigate_to_url',
            () => waitForNavigation(tabId, step.wait, step.timeout_ms || step.timeoutMs || 15000)
          );
          ...
          try {
            await guardedBrokerSideEffect(
              brokerLease,
              'ensureContentReady workflow navigate_to_url',
              () => ensureContentReady(tabId, 'contentScript.js', 8000)
            );
        ...
        try {
          ...
          const url = await guardedBrokerSideEffect(
            brokerLease,
            'waitForNavigationOrUrlMatch workflow',
            () => waitForNavigationOrUrlMatch(tabId, urlPattern, timeoutMs)
          );
        ...
        try {
          ...
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete workflow wait_for_network_idle',
            () => waitForTabComplete(tabId, maxWait).catch(() => {})
          );
          const spent = Date.now() - start;
          const remaining = Math.max(0, maxWait - spent);
          await guardedBrokerSideEffect(
            brokerLease,
            'idle delay workflow wait_for_network_idle',
            () => new Promise(r => setTimeout(r, Math.min(idle, remaining)))
          );
        ...
        try {
          ...
          await guardedBrokerSideEffect(
            brokerLease,
            'forceDetachCDP workflow upload_file',
            () => forceDetachCDP(tabId).catch(() => {
        ...
        try {
          ...
          console.log(`Using workflow tab ID for get_page_source: ${tabId}`);
          const targetTab = await guardedBrokerSideEffect(
            brokerLease,
            'chrome.tabs.get workflow get_page_source',
            () => chrome.tabs.get(tabId)
          );
          ...
          await guardedBrokerSideEffect(
            brokerLease,
            'waitForTabComplete workflow get_page_source',
            () => waitForTabComplete(tabId, Math.min(timeoutMs, 15000)).catch(() => {})
          );
          await guardedBrokerSideEffect(
            brokerLease,
            'ensureContentReady workflow get_page_source',
            () => ensureContentReady(tabId, 'contentScript.js', 8000).catch(() => {})
          );
          ...
          for (let attempt = 0; attempt < 3; attempt++) {
            try {
              [result] = await guardedBrokerSideEffect(
                brokerLease,
                'chrome.scripting.executeScript workflow get_page_source',
                () => chrome.scripting.executeScript({
                  target: { tabId: tabId },
                  func: async () => {
              ...
              await guardedBrokerSideEffect(
                brokerLease,
                'retry delay workflow get_page_source',
                () => new Promise(r => setTimeout(r, 250))
              );
              await guardedBrokerSideEffect(
                brokerLease,
                'retry waitForTabComplete workflow get_page_source',
                () => waitForTabComplete(tabId, 3000).catch(() => {})
              );
            }
          }
          ...
          try {
            const [pruned] = await guardedBrokerSideEffect(
              brokerLease,
              'chrome.scripting.executeScript workflow pruneDOM',
              () => chrome.scripting.executeScript({ target: { tabId: tabId }, func: pruneDOM })
            );
          ...
          try {
            const domSnapshotResponse = await guardedBrokerSideEffect(
              brokerLease,
              'sendMessageTopFrame workflow get_dom_snapshot',
              () => sendMessageTopFrame(tabId, attachBrokerLeaseToMessage({
                cmd: 'get_dom_snapshot',
                req_id: requestId,
                payload: {
        ...
        try {
          ...
          if (workflowTabId === maybeTabId) {
            commitWorkflowTabIdForLease(
              ...
              setSessionWorkflowTabId,
              undefined,
              'clear workflow close_current_tab'
            );
          }
        ...
        try {
          ...
          if (wantAnnotate) {
            try {
              ...
              await guardedBrokerSideEffect(
                brokerLease,
                'waitForTabComplete workflow screenshot annotate',
                () => waitForTabComplete(workflowTabId!, 15000).catch(() => {})
              );
              await guardedBrokerSideEffect(
                brokerLease,
                'ensureContentReady workflow screenshot annotate',
                () => ensureContentReady(workflowTabId!, 'contentScript.js', 8000).catch(() => {})
              );

              const domResp: any = await guardedBrokerSideEffect(
                brokerLease,
                'sendMessageTopFrameWithRetry workflow screenshot annotate',
                () => sendMessageTopFrameWithRetry(
                  workflowTabId!,
                  attachBrokerLeaseToMessage(
                    {
                      cmd: 'get_dom_snapshot',
                      req_id: `${requestId}-dom`,
                      payload: { options: { maxElements, highlightElements: false } },
      ...
      if (workflowTabId !== undefined) {
        ...
        tabId = await guardedBrokerSideEffect(
          brokerLease,
          'get active tab workflow content-script step',
          () => getActiveTabIdOrThrow(step.type || 'content-script step')
        );
        commitWorkflowTabIdForLease(
          brokerLease,
          setSessionWorkflowTabId,
          tabId,
          'bind workflow content-script step'
        );
      }
      ...
      const tab = await guardedBrokerSideEffect(
        brokerLease,
        'chrome.tabs.get workflow content-script step',
        () => chrome.tabs.get(tabId)
      );
      ...
      try {
        ...
        await guardedBrokerSideEffect(
          brokerLease,
          'ensureContentReady workflow content-script dispatch',
          () => ensureContentReady(tabId, 'contentScript.js', 8000)
        );
        ...
        const response = await guardedBrokerSideEffect(
          brokerLease,
          'sendMessageTopFrameWithRetry workflow content-script dispatch',
          () => sendMessageTopFrameWithRetry(tabId, stepMessage, { attempts: 3, timeoutMs: messageTimeoutForStep(step) })
        );
        if (!response.success) {
        ...
        try {
          ...
          if (maybeNavSteps.has(step.type)) {
            ...
            while (Date.now() - pollStart < 1500) {
              const after = await guardedBrokerSideEffect(
                brokerLease,
                'chrome.tabs.get workflow post-step poll',
                () => chrome.tabs.get(tabId)
              );
              if (after.status === 'loading') break;
              await guardedBrokerSideEffect(
                brokerLease,
                'post-step poll delay workflow',
            ...
            const after = await guardedBrokerSideEffect(
              brokerLease,
              'chrome.tabs.get workflow post-step settle',
              () => chrome.tabs.get(tabId)
            );
            if (after.status === 'loading') {
              await guardedBrokerSideEffect(
                brokerLease,
                'waitForNavigation workflow post-step settle',
                () => waitForNavigation(tabId, 'domcontentloaded', settleTimeout)
              );
            }
            await guardedBrokerSideEffect(
              brokerLease,
              'ensureContentReady workflow post-step settle',
              () => ensureContentReady(tabId, 'contentScript.js', 8000)
            );
          }
        } catch (e) {
    ...
    if (workflowTabId !== undefined) {
      try {
        const tab = await guardedBrokerSideEffect(
          brokerLease,
          'chrome.tabs.get workflow final current_url',
          () => chrome.tabs.get(workflowTabId!)
        );
        currentUrl = tab.url || '';
      } catch (e) {
...
function runtimeSenderSummary(sender: chrome.runtime.MessageSender): Record<string, any> {
  return {
    id: sender.id || null,
    tabId: sender.tab?.id ?? null,
    frameId: sender.frameId ?? null,
    url: sender.url ? '[redacted]' : null,
  };
}
...
function rejectRuntimeSender(
  cmd: string,
  sender: chrome.runtime.MessageSender,
  sendResponse: (response?: any) => void
): void {
  logWarn('Rejected runtime message from unauthorized sender', {
    cmd,
    sender: runtimeSenderSummary(sender),
  });
  sendResponse({
    success: false,
    ok: false,
    error: `Unauthorized sender for ${cmd}`,
  });
}

function requirePopupSender(
  cmd: string,
  sender: chrome.runtime.MessageSender,
  sendResponse: (response?: any) => void
): boolean {
  if (isPopupOrExtensionPageSender(sender)) return true;
  rejectRuntimeSender(cmd, sender, sendResponse);
  return false;
}

function requireContentSender(
  cmd: string,
  sender: chrome.runtime.MessageSender,
  sendResponse: (response?: any) => void
): boolean {
  if (isContentScriptSender(sender)) return true;
  rejectRuntimeSender(cmd, sender, sendResponse);
  return false;
}
...
if (guardListener(chrome.runtime?.onMessage, 'chrome.runtime.onMessage')) {
  ...
  if (message.cmd === 'take_screenshot') {
    ...
    (async () => {
      try {
        ...
        if (tabId === undefined) {
          throw new Error('No sender tab available for content screenshot');
        }
  ...
  if (message.cmd === 'download_image') {
    ...
    console.log('Downloading image request:', {
      ...
      has_url: typeof message.url === 'string' && message.url.length > 0,
    ...
    if (!requireContentSender(message.action, sender, sendResponse)) return false;
    // Handle first-class press_key action using CDP
    console.log('Handling press_key_cdp:', { req_id: message.req_id, has_key: !!message.key });
    ...
    console.log('Handling rzn_fetch_ax_slice:', {
      req_id: message.req_id,
    ...
    (async () => {
      await loadWorkflowSessionsFromStorage();
      const workflowSessionId = normalizeSessionId(resolveSessionId(message));
      let workflowTabId = getWorkflowTabId(workflowSessionId);
      const requestedTabId = resolveRequestedTabId(message);
      if (requestedTabId !== undefined) {
    ...
    if (!requireContentSender(message.cmd, sender, sendResponse)) return false;
    (async () => {
      sendResponse(await setFlagsForMessage(message));
    })();
