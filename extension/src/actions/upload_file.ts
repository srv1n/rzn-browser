...
async function getFileInputObjectId(handle: CdpHandle, selector: string): Promise<string> {
  const expression = `(() => {
...
async function uploadFile(
  ...
  const objectId = await getFileInputObjectId(handle, selector);
  await handle.sendCommand("DOM.enable", {});
  await handle.sendCommand("DOM.setFileInputFiles", { objectId, files: filePaths });
...
export async function handleUploadFile(step: any): Promise<any> {
  ...
  const result = await uploadFile(sessionId, targetTabId, String(selector), filePaths);
  const payload = {
