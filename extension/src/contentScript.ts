...
async function executeWithSharedRequestDedup<T>(
  ...
  if (!node) {
    ...
    node.setAttribute('data-rzn-owner', CONTENT_SCRIPT_INSTANCE_ID);
  ...
  const owner = node.getAttribute('data-rzn-owner');
    ...
    (async () => {
      try {
        const response = await executeWithSharedRequestDedup(
          ...
          const stepObj = message.payload?.step || {};
...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  if (data.type === 'RZN_TEST_PING') {
    window.postMessage({ type: 'RZN_TEST_PONG', requestId: data.requestId, payload: true }, '*');
    return;
  }
