...
# - rzn-browser + rzn-browser-worker + rzn-native-host
...
if [[ "$SKIP_RUST" != "1" ]]; then
  echo "[INFO] Building Rust (release): rzn-browser + rzn-browser-worker + rzn-native-host"
...
if [[ ! -x "target/release/rzn-browser-worker" ]]; then
  echo "[ERROR] Missing built worker at target/release/rzn-browser-worker"
  exit 1
fi
...
cp -f target/release/rzn-browser-worker "${STAGE_DIR}/bin/rzn-browser-worker"
cp -f target/release/rzn-browser "${STAGE_DIR}/bin/rzn-browser"
chmod +x "${STAGE_DIR}/bin/rzn-native-host" "${STAGE_DIR}/bin/rzn-browser-worker" "${STAGE_DIR}/bin/rzn-browser" || true
