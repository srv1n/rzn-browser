...
| `workflows/` | Shipped workflow catalog and catalog documentation |
...
| `docs/system/` | Current architecture, operations, testing, and workflow-authoring docs |
...
- The release installer writes the Chrome native messaging registration. Source builds can also register Edge and Chromium. Inspect registrations with `rzn-browser native-host list --json`.
...
- Linux Chrome: `~/.local/share/RZN/extension/dist/chrome`
- Windows Chrome: `%LOCALAPPDATA%\\RZN\\extension\\dist\\chrome`
...
For Chrome + Edge or Chromium setup, target flags, doctor output, and native-host troubleshooting, see [Installation and operations](docs/system/installation-operations.md).
...
```sh
rzn-browser workflow list                 # every system
rzn-browser workflow list linkedin        # one system's workflows
rzn-browser workflow show chatgpt send    # full help for one workflow
```
...
- `rzn-browser workflow list <system>` keeps the output compact.
...
- `--source user|builtin`: only show entries from that catalog source
...
- Run `rzn-browser workflow validate <path>` while authoring.
...
- `docs/system/workflow-authoring.md` for the workflow authoring and validation loop
...
2. Add or update `workflows/<system>/README.md` and the catalog documentation in `workflows/README.md`.
...
- A system README in `workflows/<system>/README.md`
...
For deeper setup and architecture details, see [developer guide](docs/system/developer-guide.md), [architecture](docs/system/architecture.md), and the [documentation index](docs/system/INDEX.md).
