...
`runtime.ensure_ready` waits/probes for the native-host bridge. It should return structured diagnostics rather than hiding a degraded extension state.

## Endpoint authority

`broker_endpoint_v1.json` and browser-worker pid/socket files are ignored migration debris. The supervisor socket/token pair is the runtime discovery path.
