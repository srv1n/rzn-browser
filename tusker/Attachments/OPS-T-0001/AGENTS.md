...
Tusker is the sole repo-local system for planning, task tracking, evidence, docs-impact checks, verification, and closeout.

This project's vault landing page lives at `tusker/README.md` — read it before logging new work. It has two sections: a hand-edited **Project overview** (preserved across regens, between `<!-- tusker:overview:begin -->` markers) and an auto-generated **Epic roster** (regenerated on every `tusker reindex`). For a live terminal view of epics only, run `tusker list --type epic`.
