...
|---|---|
| Cached `broker_endpoint_v1.json` pid/socket could decide availability. | Supervisor socket/token are only hints until a live authenticated handshake succeeds. |
...
| Legacy worker fallback could hide the real native-host bridge outage. | Fallback is explicit, visible, and off by default on supervisor paths. |
