...
impl SupervisorState {
    ...
    async fn ensure_ready(&self, params: Value) -> Result<Value> {
        ...
        Ok(json!({
            ...
            "error": if ready {
                ...
                json!("Native-host bridge is connected but the extension did not answer the readiness ping. Reload the RZN extension or run `rzn-browser heal --json`, then retry.")
            } else {
    ...
    async fn probe_native_bridge(&self, timeout_ms: u64) -> Value {
            ...
            .await
        {
            Ok(Some(value)) => {
                ...
                let pong = value.pointer("/result/pong").and_then(|v| v.as_bool()) == Some(true);
                json!({
                    "ok": success || pong,
    ...
    async fn runtime_heal(&self) -> Result<Value> {
        ...
        let initial_readiness = self
            .ensure_ready(json!({
                "bridge_wait_ms": 500,
                "bridge_probe_timeout_ms": 1_500
            }))
            .await?;
        let final_readiness = if initial_readiness
            .get("ready")
            .and_then(|value| value.as_bool())
            == Some(true)
        {
...
fn with_run_result(tool_name: &str, mut response: Value, include_debug: bool) -> Value {
    ...
    if let Value::Object(map) = &mut response {
        map.entry("run_result".to_string()).or_insert(run_result);
    }
    response
}
...
fn side_effects_for_step_type(step_type: &str) -> &'static [SideEffectClassV2] {
    match step_type {
        "get_page_source" | "get_element_text" | "wait_for_element" | "wait_for_timeout" => {
            &[SideEffectClassV2::ReadOnly]
        }
        "navigate_to_url" => &[SideEffectClassV2::BrowserState],
        "click_element" | "click" | "fill_input_field" | "type_text" | "press_special_key"
        | "press_key" => &[SideEffectClassV2::BrowserState],
        "upload_file" => &[
        ...
        "execute_javascript" | "javascript" | "eval" => &[SideEffectClassV2::BrowserState],
