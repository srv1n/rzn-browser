...
| --- | --- | --- |
| `crates/rzn_browser/src/cloud.rs` | dev-harness cloud control plane CLI/server | keep as local harness; product actor polling moves behind supervisor client APIs |
| `crates/rzn_native_host/src/cloud.rs` | native-host-owned cloud actor loop | migrate actor config, lease polling, dedupe, and result reporting to supervisor cloud module |
...
This is the concrete cutover contract for `LRT-T-0005`.

| Step | Source today | Target after supervisor IPC lands | Guardrail |
|---|---|---|---|
| Actor config read/write | `crates/rzn_native_host/src/cloud.rs` and extension popup local controls. | Supervisor cloud module; extension/native host forward configure/status commands to supervisor. | Refresh tokens never enter extension storage. |
| Lease loop | Native-host-owned WebSocket dev actor. | Supervisor-owned outbound OAuth polling loop. | Native host can restart without dropping cloud actor ownership. |
| Dedupe | Dev harness result cache around command ids. | Supervisor spool + terminal command result cache keyed by `command_id`. | `lease_id` is an attempt id, not an extension request id. |
...
| Status | Native-host `cloud_get_status`. | `cloud.status` in `rzn.local.v1`; extension popup can call through a forwarding control during migration. | Status says `degraded` when extension/native bridge is absent, not `offline` if supervisor is healthy. |
...
5. Fake dispatcher tests should cover: initial lease -> ack -> dispatch -> result, duplicate `command_id` with new `lease_id`, supervisor restart after ack before result, and extension unavailable at dispatch time.
