...
async function runScriptingEval(
  ...
  const tabId = explicitTabId ?? sender?.tab?.id ?? await getActiveBrowserTabId('script eval');
  if (tabId === undefined) {
...
async function runCdpEval(
  ...
  const explicitTabId = typeof message?.tabId === 'number' ? message.tabId : undefined;
  const tabId = explicitTabId ?? sender?.tab?.id ?? await getActiveBrowserTabId('CDP eval');
  if (tabId === undefined) {
...
async function handleBrokerMessage(message: BrokerMessage, sessionId?: string): Promise<void> {
  ...
  if (message.cmd === 'enable_debug') {
    try {
      ...
      if (tabId === undefined) {
        try {
          tabId = await getActiveBrowserTabId('enable_debug');
        } catch {
          sendResponseToBroker({
            req_id: requestId,
            success: false,
  ...
  if (message.cmd === 'disable_debug') {
    try {
      ...
      if (workflowTabId !== undefined) {
        ...
        try {
          tabId = await getActiveBrowserTabId('disable_debug');
        } catch {
          sendResponseToBroker({
            req_id: requestId,
            success: false,
    ...
    try {
      ...
      if (workflowTabId === undefined && requiresExistingTab) {
        if (mayUseCurrentTab) {
          ...
          tabId = await getActiveTabIdOrThrow('use_current_tab execute_step');
          setSessionWorkflowTabId(tabId);
          await waitForTabComplete(tabId, 15000).catch(() => {});
          ...
          try {
            newTab = await chrome.tabs.create({ url: 'https://www.example.com', active: true });
          } catch (err: any) {
            ...
            const newWindow = await chrome.windows.create({ url: 'https://www.example.com', focused: true });
  ...
  if (requiresExistingTab && workflowTabId === undefined) {
    ...
    try {
      if (preferCurrentTab) {
        ...
        workflowTabId = await getActiveTabIdOrThrow('use_current_tab workflow');
  ...
  try {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      try {
        ...
        const response = await sendMessageTopFrameWithRetry(tabId, stepMessage, { attempts: 3, timeoutMs: messageTimeoutForStep(step) });
        if (!response.success) {
  ...
  if (message.cmd === 'take_screenshot') {
    (async () => {
      try {
        const tabId =
          sender?.tab?.id ??
          getWorkflowTabId(DEFAULT_WORKFLOW_SESSION_ID) ??
          await getActiveBrowserTabId('screenshot');
