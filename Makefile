...
.PHONY: help build build-rust build-ext clean logs-clear logs-follow logs-show test test-basic test-google test-dom test-dom-units dev setup install reload-ext \
...
help:
	...
	@echo ""
	@echo "Setup Commands:"
	@echo "  make install       - Release install (CLI + worker + native host + stable extension copy + bundled workflows/examples)"
...
build-rust:
	...
	RZN_LOG_ENABLED=1 cargo build --release
...
# Validate local wiring (native host manifest + /tmp/rzn.sock)
doctor:
	@bash scripts/cli_factory_doctor.sh
...
phase3:
	...
	LLM_PROVIDER=dummy cargo build --release -p rzn-browser -p rzn-browser-worker -p rzn-native-host
