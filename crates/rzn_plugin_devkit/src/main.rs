...
fn parse_mode(raw: &Option<serde_json::Value>) -> Result<u32> {
    ...
    let Some(s) = v.as_str() else {
        bail!("payload mode must be an octal string");
    };
    let trimmed = s.trim();
    if trimmed.is_empty() || !trimmed.chars().all(|c| matches!(c, '0'..='7')) {
        bail!("payload mode must be an octal string");
    }
    u32::from_str_radix(trimmed, 8).context("parse octal mode")
}
...
fn is_reserved_bundle_entry(name: &str) -> bool {
    matches!(name, "plugin.json" | "plugin.sig")
}
...
fn verify_bundle_zip(zip_path: &Path, public_key_path: &Path) -> Result<()> {
    ...
    for i in 0..archive.len() {
        ...
        if !is_safe_relative_path(&entry_name) {
            bail!("unsafe zip entry path {}", entry_name);
        }
    ...
    for entry_name in &entry_names {
        if is_reserved_bundle_entry(entry_name) || allow.contains(entry_name) {
            continue;
        }
        bail!("zip entry not listed in manifest: {}", entry_name);
    }
    for key in &allow {
...
mod tests {
    ...
    fn write_public_key(temp: &TempDirGuard, signing_key: &SigningKey) -> Result<PathBuf> {
        let path = temp.path.join("ed25519.public");
        fs::write(
            &path,
    ...
    fn signed_manifest(
        ...
        let manifest = PluginManifestV1 {
            ...
            workers: Vec::new(),
            resources: Vec::new(),
            sha256: BTreeMap::from([(payload_path.to_string(), sha256_hex(payload_bytes))]),
        };
        let manifest_bytes = serialize_manifest_bytes(&manifest)?;
        let sig_b64 = sign_manifest_b64(signing_key, &manifest_bytes);
        Ok((manifest_bytes, sig_b64))
    }
    ...
    fn write_test_zip(
        ...
        let opts = FileOptions::<()>::default()
            .compression_method(zip::CompressionMethod::Deflated)
            .last_modified_time(fixed_zip_date_time())
            .unix_permissions(0o644);

        zw.start_file("plugin.json", opts)?;
        zw.write_all(manifest_bytes)?;
        zw.start_file("plugin.sig", opts)?;
        zw.write_all(sig_b64.as_bytes())?;
        zw.write_all(b"\n")?;
    ...
    #[test]
    fn parse_mode_accepts_octal_strings_only() -> Result<()> {
        assert_eq!(parse_mode(&None)?, 0o644);
        assert_eq!(
            parse_mode(&Some(JsonValue::String("755".to_string())))?,
            0o755
        );

        let numeric_err = parse_mode(&Some(JsonValue::from(755))).unwrap_err();
        assert!(
            format!("{numeric_err:#}").contains("payload mode must be an octal string"),
            "{numeric_err:#}"
        );

        let decimal_err = parse_mode(&Some(JsonValue::String("888".to_string()))).unwrap_err();
        assert!(
            format!("{decimal_err:#}").contains("payload mode must be an octal string"),
            "{decimal_err:#}"
        );
        Ok(())
    }
    ...
    #[test]
    fn correct_bundle_verifies() -> Result<()> {
        let temp = TempDirGuard::new("rzn-plugin-devkit-verify-good")?;
        let signing_key = SigningKey::generate(&mut OsRng);
        let public_key = write_public_key(&temp, &signing_key)?;
        let payload = b"worker-binary";
        let (manifest_bytes, sig_b64) = signed_manifest(&signing_key, "bin/worker", payload)?;
        ...
        write_test_zip(
            ...
            &manifest_bytes,
            &sig_b64,
            vec![("bin/worker", payload.to_vec())],
        )?;

        verify_bundle_zip(&zip_path, &public_key)?;
        Ok(())
    }

    #[test]
    fn verify_rejects_zip_entry_not_listed_in_manifest() -> Result<()> {
        let temp = TempDirGuard::new("rzn-plugin-devkit-verify-extra")?;
        let signing_key = SigningKey::generate(&mut OsRng);
        let public_key = write_public_key(&temp, &signing_key)?;
        let payload = b"worker-binary";
        let (manifest_bytes, sig_b64) = signed_manifest(&signing_key, "bin/worker", payload)?;
        ...
        write_test_zip(
            ...
            &sig_b64,
            vec![
                ("bin/worker", payload.to_vec()),
                ("bin/extra", b"not in manifest".to_vec()),
            ],
        )?;

        let err = verify_bundle_zip(&zip_path, &public_key).unwrap_err();
        let msg = format!("{err:#}");
        assert!(msg.contains("zip entry not listed in manifest"), "{msg}");
        assert!(msg.contains("bin/extra"), "{msg}");
        Ok(())
    }

    #[test]
    fn verify_rejects_traversal_zip_entry_before_hashing() -> Result<()> {
        let temp = TempDirGuard::new("rzn-plugin-devkit-verify-traversal")?;
        let signing_key = SigningKey::generate(&mut OsRng);
        let public_key = write_public_key(&temp, &signing_key)?;
        let expected_payload = b"worker-binary";
        let (manifest_bytes, sig_b64) =
            signed_manifest(&signing_key, "bin/worker", expected_payload)?;
        let zip_path = temp.path.join("bundle.zip");
        ...
        write_test_zip(
            ...
            &sig_b64,
            vec![
                ("bin/worker", b"tampered-binary".to_vec()),
                ("../evil", b"owned".to_vec()),
            ],
        )?;
        ...
        let err = verify_bundle_zip(&zip_path, &public_key).unwrap_err();
        let msg = format!("{err:#}");
        assert!(msg.contains("unsafe zip entry path ../evil"), "{msg}");
    ...
    fn directory_resources_and_examples_are_hashed_into_manifest() -> Result<()> {
        ...
        let worker_bin = temp.path.join("artifacts/plugin-worker");
        ...
        let config = BundleConfig {
            ...
            workers: vec![WorkerConfig {
                ...
                entrypoints: BTreeMap::from([(
                    ...
                    "bin/macos/universal/plugin-worker".to_string(),
                )]),
                args: Vec::new(),
                env: BTreeMap::new(),
            }],
            ...
            payloads: vec![
                ...
                PayloadConfig {
                    ...
                    dest: "bin/macos/universal/plugin-worker".to_string(),
                    platforms: Some(vec!["macos_universal".to_string()]),
                    mode: Some(JsonValue::String("755".to_string())),
                },
                PayloadConfig {
