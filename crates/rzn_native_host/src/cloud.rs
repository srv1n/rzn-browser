...
fn normalize_local_actor_config(
    ...
    config.websocket_url = if config.websocket_url.trim().is_empty() {
        ...
        normalize_websocket_url(&config.websocket_url)?
    };
...
fn resolve_cloud_actor_config_path() -> PathBuf {
    if let Ok(path) = std::env::var("RZN_CLOUD_CONFIG_PATH") {
        ...
        tracing::warn!(
            "Ignoring RZN_CLOUD_CONFIG_PATH outside the per-user cloud config directory: {}",
            candidate.display()
        );
    }
...
fn is_allowed_cloud_config_path(path: &std::path::Path) -> bool {
...
fn normalize_path_for_prefix(path: &std::path::Path) -> Option<PathBuf> {
...
mod tests {
    ...
    fn actor_ws_request_sends_token_in_header_not_url() {
        ...
        assert!(!uri.contains("actor_token"));
        assert_eq!(
            request
                .headers()
                .get(AUTHORIZATION)
