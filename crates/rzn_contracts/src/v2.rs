...
pub fn format_tab_ref(
    ...
    Ok(TabRefV1::new(browser_instance_id, tab_id)?.as_ref_string())
}

pub fn parse_tab_ref(value: &str) -> Result<TabRefV1, String> {
    TabRefV1::parse(value)
}
...
mod tests {
    ...
    fn tab_ref_round_trips_and_distinguishes_browser_instances() {
        let chrome = TabRefV1::new("chrome-instance", 123).expect("tab ref");
        let edge = TabRefV1::new("edge-instance", 123).expect("tab ref");
        ...
        assert_eq!(
            TabRefV1::parse(&chrome.as_ref_string()).expect("parse"),
            chrome
        );
