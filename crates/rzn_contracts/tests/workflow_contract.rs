...
#[test]
fn manifest_roundtrips_and_validates_side_effects() {
    let manifest = fixture_manifest();

    let raw = serde_json::to_string(&manifest).expect("serialize manifest");
    let back: WorkflowManifest = serde_json::from_str(&raw).expect("deserialize manifest");
...
#[test]
fn manifest_rejects_undeclared_step_side_effects() {
    let mut manifest = fixture_manifest();
    manifest.steps[0]
        .action
        .side_effects
        .push(SideEffectClass::ExternalWrite);
...
#[test]
fn manifest_rejects_zero_retry_attempts() {
    let mut manifest = fixture_manifest();
...
fn side_effect_classes_include_external_read_and_network_access() {
    assert_eq!(
        serde_json::to_value(SideEffectClass::ExternalRead).expect("serialize"),
        json!("external_read")
    );
    assert_eq!(
        serde_json::to_value(SideEffectClass::NetworkAccess).expect("serialize"),
        json!("network_access")
    );
}
...
fn registered_engine_action_kinds_roundtrip() {
    ...
    for kind in action_kinds {
        let decoded: WorkflowActionKind =
...
#[test]
fn params_apply_defaults_coerce_and_reject_unknowns() {
    let manifest = fixture_manifest();
    let params = manifest
...
#[test]
fn run_request_validates_against_manifest_identity_and_params() {
    let manifest = fixture_manifest();
    let mut params = Map::new();
    params.insert("query".to_string(), json!("rust"));
    params.insert("limit".to_string(), json!(3));

    let envelope = RunRequest {
        version: RUN_REQUEST_CONTRACT.to_string(),
        ...
        params,
        policy: RunPolicy {
            allow_side_effects: vec![SideEffectClass::BrowserState],
            dry_run: false,
        },
    };
