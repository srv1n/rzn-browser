...
async fn issue_pairing_code(args: CloudIssuePairingCodeArgs) -> Result<()> {
    ...
    let response: IssuePairingCodeResponse = reqwest::Client::new()
        .post(format!(
            "{}/v1/pairing-codes",
...
async fn pair_actor(args: CloudPairArgs) -> Result<()> {
    ...
    let response: RedeemPairingResponse = reqwest::Client::new()
    ...
    let config = LocalCloudActorConfig {
        ...
        server_url: response.server_url.clone(),
...
async fn run_workflow(args: CloudRunWorkflowArgs) -> Result<()> {
    ...
    let response: RunWorkflowResponse = reqwest::Client::new()
        .post(format!(
            "{}/v1/runs/workflow",
            normalize_server_url(&args.server)?
        ))
...
async fn list_actors(args: CloudListActorsArgs) -> Result<()> {
    let response: ListActorsResponse = reqwest::Client::new()
...
async fn ws_actor_connect(
    ...
    if !state
        .actor_is_authorized(&query.actor_id, &query.actor_token)
        .await
    {
        return Err(ApiError::new(
...
fn persist_local_actor_config(path: &FsPath, config: &LocalCloudActorConfig) -> Result<()> {
    ...
    std::fs::write(path, bytes).with_context(|| format!("Write actor config {}", path.display()))
}
...
mod tests {
    ...
    #[test]
    fn websocket_url_uses_ws_or_wss_scheme() {
        assert_eq!(
