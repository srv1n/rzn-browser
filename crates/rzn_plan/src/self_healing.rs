...
impl SelfHealer {
    ...
    fn generate_alternative_selector(&self, original: &str) -> String {
        ...
        if let Some(id) = original.strip_prefix('#') {
            // If it's an ID selector, try a more generic approach
            format!("[id='{}'], [id*='{}']", id, id)
        } else if let Some(class) = original.strip_prefix('.') {
            ...
            "button, [role='button'], input[type='button'], input[type='submit']".to_string()
        } else if original.contains("input") {
            // For input selectors, try by type and name
            "input, [type='text'], [type='email'], [type='password']".to_string()
        } else {
