...
impl PromptBuilder {
    ...
    pub fn build_dom_aware_planning_prompt_with_recovery(
        ...
        use std::fmt::Write;
        let _ = writeln!(user_content, "Goal: {}", goal);
        ...
        if let Some(context) = dom_context {
            ...
            if !context.semantic_groups.is_empty() {
                context_content.push_str("Available interactions:\n");
                for (group, selectors) in &context.semantic_groups {
                    let _ = writeln!(context_content, "- {}: {} elements", group, selectors.len());
                }
                context_content.push('\n');
            }

            // Add key interactive elements with recommendations
            context_content.push_str("Key interactive elements:\n");
            for (i, element) in context.interactive_elements.iter().take(15).enumerate() {
                ...
                if element.selector_suggestions.len() > 1 {
                    ...
                    context_content.push_str("   Alternatives: ");
                    let mut first = true;
                    for alt in alternatives {
                        if !first {
                            context_content.push_str(", ");
                        }
                        context_content.push_str(alt);
                        first = false;
                    }
                    if !first {
                        context_content.push('\n');
            ...
            push_untrusted_block(&mut user_content, "PAGE CONTEXT:", &context_content);
        } else {
            user_content.push_str(
                "Current State: No active tab - MUST start with navigate_to_url WITH A VALID URL\n",
            );
                    ...
                    .contains("Page Inspection for Failed Extraction")
                {
                    ...
                    } = &execution.result
                    {
                                        ...
                                        if let Some(selector) = discovered_item.get("selector") {
                                            if let Some(count) = discovered_item.get("count") {
                                                    ...
                                                    discovered_item.get("sampleText")
                                                {
                                                    ...
                                                    user_content.push_str(&format!(
                                                        "{}\n",
                                                        wrap_untrusted_content(&discovered_line)
                                                    ));
                                                } else {
                                                    let discovered_line = format!(
                                                        "      - {}: {} items",
                        ...
                        if let Some(page_title) = inspection_data.get("pageTitle") {
                            let page_title_line = format!(
                                "    Page Title: \"{}\"",
                                page_title.as_str().unwrap_or("Unknown")
                            );
                            user_content.push_str(&format!(
                                "{}\n",
                                wrap_untrusted_content(&page_title_line)
                            ));
                        }
                        ...
                        if let Some(original_selector) = inspection_data.get("originalSelector") {
                            ...
                            user_content
                                .push_str(&format!("{}\n", wrap_untrusted_content(&selector_line)));
                        }
                                    ...
                                    if let Some(debug_obj) = value.as_object() {
                                            ...
                                            debug_obj.get("availableSelectors")
                                        {
                                                ...
                                                available_selectors.as_array()
                                            {
                                                ...
                                                user_content.push_str(&format!(
                                                    "{}\n",
                                                    wrap_untrusted_content(&debug_line)
                                                ));
                                            }
                                        }
                                    }
        ...
        if let Some(tracker) = failure_tracker {
            if tracker.consecutive_failures > 0 {
                ...
                let _ = writeln!(user_content, "{}", tracker.get_error_summary());
...
- One action at a time - be patient like a human"#,
    ...
    pub fn build_planner_prompt(
        ...
        let whitelist_summary = whitelist.generate_summary();
        push_untrusted_block(&mut user_content, "WHITELIST ELEMENTS:", &whitelist_summary);
    ...
    pub fn build_compact_snapshot_prompt(
        ...
        if !snapshot.elements.is_empty() {
            snapshot_content.push_str("Interactive Elements:\n");
            ...
            snapshot_content.push_str("No interactive elements found.\n");
        }

        push_untrusted_block(&mut user_content, "PAGE SNAPSHOT:", &snapshot_content);

        // Add memory summary if available
        if let Some(memory) = &snapshot.memory_summary {
            push_untrusted_block(&mut user_content, "Recent Actions:", memory);
        }
        ...
        if let Some(cache) = failure_cache {
            ...
            if !failure_summary.trim().is_empty() {
                push_untrusted_block(&mut user_content, "Known Issues:", &failure_summary);
            }
        }
    ...
    pub fn build_navigator_prompt(
            ...
            .and_then(|i| i.as_u64())
        {
            if let Some(element) = whitelist.get_element(index as u32) {
                let mut element_details = String::new();
                element_details.push_str(&format!("  Tag: {}\n", element.tag));
                element_details.push_str(&format!("  Primary Selector: {}\n", element.selector));
                element_details.push_str(&format!("  Text: \"{}\"\n", element.text));
                element_details.push_str(&format!("  Confidence: {:.2}\n", element.confidence));
                element_details.push_str(&format!("  Visible: {}\n", element.visible));

                if !element.alt_selectors.is_empty() {
                    element_details.push_str("  Alternative Selectors:\n");
                    for alt in &element.alt_selectors {
                        element_details.push_str(&format!("    - {}\n", alt));
                    }
                }

                if !element.attributes.is_empty() {
                    element_details.push_str("  Key Attributes:\n");
    ...
    pub fn build_summary_prompt(
        ...
        let mut user_content = String::new();
        user_content.push_str("WORKFLOW SUMMARY REQUEST\n\n");
        user_content.push_str(&format!("Goal: {}\n", goal));
        user_content.push_str(&format!(
            "Status: {}\n",
            if success {
