...
impl LLMClient {
    ...
    pub fn new_simple(api_key: String) -> Self {
        // Get model from environment variable
        let model =
            std::env::var("OPENAI_MODEL_PLANNING").unwrap_or_else(|_| "gpt-4o-mini".to_string());

        let provider = Arc::new(
            OpenAIClient::new(api_key, model, 30)
                .unwrap_or_else(|_| panic!("Failed to create OpenAI client")),
        );
    ...
    pub async fn chat_json(
        ...
        if is_openai {
            ...
            // 3) As a last resort, scan the whole response for a JSON object
            let response_str = response.to_string();
            if let Some(start) = response_str.find('{') {
                if let Some(end) = response_str.rfind('}') {
                    let slice = &response_str[start..=end];
        ...
        if let Some(home) = dirs::home_dir() {
            let path = home.join("rzn_build.log");
            if let Ok(mut f) = OpenOptions::new().create(true).append(true).open(path) {
                ...
                let _ = writeln!(
                    ...
                    serde_json::json!({
                        ...
                        "level": "DEBUG",
                        "component": "llm_raw",
                        "message": "openai.chat_completion response",
                        "data": response
                    })
                    .to_string()
                );
        ...
        if content.is_empty() {
            // Log the full response again if content is empty
            if let Some(home) = dirs::home_dir() {
                let path = home.join("rzn_build.log");
                if let Ok(mut f) = OpenOptions::new().create(true).append(true).open(path) {
                    let ts = chrono::Utc::now().to_rfc3339();
                        ...
                        if in_string {
                            ...
                            if b == b'"' {
                                ...
                                if depth == 0 {
                                    ...
                                    if let Ok(v) =
                                        serde_json::from_slice::<Value>(&bytes[start..=j])
                                    {
                                        return Some(v);
                                    }
                                    break;
    ...
    pub async fn plan_next_step(&self, messages: Vec<Value>) -> PlanResult<LLMResponse> {
        ...
        // Log the full messages being sent to a file
        let log_path = dirs::home_dir()
            .map(|p| p.join("rzn_llm_messages.log"))
            .unwrap_or_else(|| std::path::PathBuf::from("rzn_llm_messages.log"));

        if let Ok(mut file) = OpenOptions::new().create(true).append(true).open(&log_path) {
            let _ = writeln!(file, "\n{}", "═".repeat(80));
            let _ = writeln!(
                file,
                " FULL MESSAGES TO LLM (Planning) [{}]",
            ...
            let _ = writeln!(file, "{}", messages_str);
            let _ = writeln!(file, "{}", "═".repeat(80));
            let _ = file.flush();
        }
    ...
    async fn parse_llm_response(&self, response: Value) -> PlanResult<LLMResponse> {
        ...
        if let Some(error) = response.get("error") {
            ...
            if error_type == "invalid_request_error" && error_msg.contains("API key") {
                return Err(PlanError::LLMError(format!("Invalid OpenAI API key. Please check your OPENAI_API_KEY environment variable.")));
            }
    ...
    async fn parse_group_selection(&self, response: Value) -> PlanResult<GroupSelection> {
        ...
        let arguments: Value =
            serde_json::from_str(arguments_str).map_err(|e| PlanError::SerializationError(e))?;
...
mod tests {
    ...
    async fn chat_json_retries_on_non_json_content() {
        ...
        let v = llm.chat_json(messages, None).await.expect("chat_json");
