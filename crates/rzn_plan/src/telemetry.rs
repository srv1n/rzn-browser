...
fn default_gpt4o_fallback_rate() -> CostRate {
    CostRate {
...
impl TelemetryCollector {
    ...
    pub async fn finalize_session(
        ...
        let summary_json =
            serde_json::to_string_pretty(&summary).map_err(PlanError::SerializationError)?;
...
impl CostTracker {
    ...
    pub fn calculate_cost(&self, model: &str, prompt_tokens: u32, completion_tokens: u32) -> f64 {
        if let Some(rate) = self.model_rates.get(model) {
            ...
            // Fallback to GPT-4o rates for unknown models. If a future rate table
            // removes that key, keep cost accounting alive with the built-in default.
            let fallback_rate = self.model_rates.get("gpt-4o").cloned().unwrap_or_else(|| {
                warn!("Fallback cost rate gpt-4o missing; using built-in default");
                default_gpt4o_fallback_rate()
            });
...
impl TraceWriter {
    ...
    pub async fn write_trace(&mut self, trace: &AutonomousTrace) -> PlanResult<()> {
        if let Some(ref mut file) = self.file {
            let json_line = serde_json::to_string(trace).map_err(PlanError::SerializationError)?;

            file.write_all(json_line.as_bytes())
                .await
                .map_err(PlanError::IoError)?;
            file.write_all(b"\n").await.map_err(PlanError::IoError)?;
            file.flush().await.map_err(PlanError::IoError)?;

            self.entries_written += 1;

            if self.entries_written.is_multiple_of(10) {
                debug!(
...
impl TraceReplay {
    ...
    pub async fn list_sessions(&self) -> PlanResult<Vec<SessionSummary>> {
        ...
        while let Some(entry) = entries.next_entry().await.map_err(PlanError::IoError)? {
            if entry
                .file_type()
                .await
                .map_err(PlanError::IoError)?
    ...
    pub async fn load_session_traces(&self, session_id: &str) -> PlanResult<Vec<AutonomousTrace>> {
        ...
        while let Some(month_entry) = month_dirs.next_entry().await.map_err(PlanError::IoError)? {
            if month_entry
                .file_type()
                .await
                .map_err(PlanError::IoError)?
                .is_dir()
            {
                ...
                if trace_file.exists() {
                    let file = File::open(trace_file).await.map_err(PlanError::IoError)?;
                    let reader = BufReader::new(file);
                    let mut lines = reader.lines();

                    while let Some(line) = lines.next_line().await.map_err(PlanError::IoError)? {
...
mod tests {
    ...
    #[tokio::test]
    async fn fix_telemetry_unknown_model_fallback_survives_missing_gpt4o_rate() {
        let mut cost_tracker = CostTracker::new_with_default_rates(None, vec![]);
        cost_tracker.model_rates.remove("gpt-4o");

        let cost = cost_tracker.calculate_cost("unknown-model", 1000, 500);

        assert_eq!(cost, 2.5 + 5.0);
    }
