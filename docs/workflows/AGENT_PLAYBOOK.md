...
| CLI hangs on "Timed out waiting for native host connection" | Supervisor is up but the browser/native-host bridge is not connected. | Open the target browser with the RZN extension enabled, then run `rzn-browser heal --json`. |
