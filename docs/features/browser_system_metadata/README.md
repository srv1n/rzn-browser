...
- `crates/rzn_browser/src/supervisor.rs`
  Reports supervisor/browser health so install-time diagnostics can prove the bundle is usable before
  the desktop loader consumes it.
...
- `rzn.supervisor.health` is the install-time health surface for the packaged browser system.
...
- [x] Expose packaged metadata/example inventory through supervisor/plugin health surfaces
