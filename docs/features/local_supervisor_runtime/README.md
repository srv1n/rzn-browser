...
|---|---|---|---|---|
| CLI | `rzn-browser run ...` calls `runtime.ensure_ready`, then `browser.*` methods. | Job submission, output formatting, local heal request. | Durable runtime state or extension reachability. | CLI run depends on supervisor/native-host bridge readiness, not cached endpoint files. |
...
| Historical endpoint file present | Endpoint file is ignored as authority. |

### Source anchors

- Current MCP browser tool surface: `crates/rzn_browser/src/mcp_browser.rs`.
- Current supervisor workflow runner and heal surface: `crates/rzn_browser/src/native_runner.rs`, `crates/rzn_browser/src/main.rs`.
...
- `rzn-browser mcp browser` should expose stable browser tool names: `browser.session_open`, `browser.snapshot`, `browser.execute_step`, `browser.poll_events`, plus health.
...
- `rzn-browser heal` should operate in layers: verify supervisor IPC, start supervisor, verify native-host bridge, verify extension bridge, then optionally warm a browser session.
- Historical `broker_endpoint_v1.json` files are ignored by runtime discovery.

### Native-host bridge slice

- `crates/rzn_native_host/src/main.rs` treats upstream connections as supervisor runtime bridge endpoints.
- The supervisor endpoint is discovered from the installed app base layout (`run/rzn-supervisor.sock` plus `secure/rzn-supervisor-token-v1`) and can also be overridden by `RZN_LOCAL_RUNTIME_SOCKET_PATH` / `RZN_LOCAL_RUNTIME_TOKEN_PATH` or `RZN_SUPERVISOR_SOCKET_PATH` / `RZN_SUPERVISOR_TOKEN_PATH`. Native-host attempts a framed JSON `runtime.hello` handshake and advertises itself as `native_host_bridge`.
...
- Chrome native messaging framing remains unchanged. The native-host status command `runtime_bridge_get_status` reports the supervisor bridge as preferred when the supervisor socket/token exist.
...
- `runtime.ensure_ready` reports separate readiness fields for supervisor and native-host bridge instead of treating stale process artifacts as proof of browser readiness.
...
- Native attach/spawn worker healing was removed with the worker runtime; authority is supervisor handshakes.
