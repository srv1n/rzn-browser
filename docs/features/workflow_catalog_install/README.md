...
workflow resolver
  ...
  └─ legacy ~/.rzn/workflows fallback?
   |
   v
resolved JSON path -> run / native-run / desktop-run / run-workflow
```
...
- CLI wiring:
  - [crates/rzn_browser/src/main.rs](../../../../crates/rzn_browser/src/main.rs): preferred `run` plus compatibility aliases `native-run`, `desktop-run`, and `run-workflow` accept `<system> <workflow>` or `system/workflow`; `workflow list|catalog|dirs|add|pull` expose and refresh the installed catalog.
- Planner defaults:
...
- `rzn-browser run`, `rzn-browser native-run`, `rzn-browser desktop-run`, and `rzn-browser run-workflow` accept `<system> <workflow>`, `system/workflow`, legacy flat aliases, or direct file paths.
