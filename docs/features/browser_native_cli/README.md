...
## Overview
- Goal: provide a fast CLI workflow harness that talks to `rzn-browser-worker` over the rzn-browser runtime (attach if running, else spawn) without rebuilding the desktop app, and provide a one-shot `make install` path that installs the native pieces cleanly for local distribution.
...
```
attach-or-spawn:
  read APP_BASE/secure/broker_endpoint_v1.json
    ├─ connect (pipe) → handshake → MCP initialize → tools/call
...
- Session isolation: spawned workflow sessions are expected to own dedicated tabs. Background helpers must fail closed when a non-default session has no workflow tab instead of silently stealing the user's active tab. `use_current_tab` remains the explicit opt-in escape hatch for review flows.
...
- [x] Keep spawned workflow sessions isolated to dedicated tabs unless `use_current_tab` is explicitly requested
