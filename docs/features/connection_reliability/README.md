...
**Status:** partially implemented; worker-centered reliability work is now superseded by the launch target in `docs/features/local_supervisor_runtime/README.md`
...
rzn-browser currently runs workflows through a three-process topology: a persistent `rzn-browser-worker` (the bridge), short-lived `rzn-native-host` processes (spawned by Chrome per native-messaging port), and the Chrome extension's MV3 service worker. The happy path works. The restart paths do not. The public-launch target is now the local supervisor architecture documented in `docs/features/local_supervisor_runtime/README.md`; this page remains the historical reliability record for the existing worker/native-host topology.
...
| 4 | Superseded: introduce a durable supervisor as the canonical local runtime owner. | Keep relying on the worker as the durable bridge. | The worker-only topology still leaves CLI, MCP, app, and cloud competing around cached endpoint authority. The launch target keeps one shipped binary but separates process roles cleanly. |
...
- Historical note: the three-process topology is the current implementation, not the launch target. Future architecture should follow `docs/features/local_supervisor_runtime/README.md`.
- The broker endpoint file (`~/Library/Application Support/rzn-browser/secure/broker_endpoint_v1.json`) is how the current CLI finds the worker. Keep it for migration compatibility, but do not make new supervisor code depend on it as authority.
